...
1. `tp ground <spec>` — emits **one** prompt (the envelope carries `prompt`, a string, where review
   ...
   `ground-<base>-rN.ndjson` as the file the unit writes.
...
3. `tp review --merge r1.ndjson ... -o merged.ndjson` — dedup across prompts. An all-empty (converged) input set still merges cleanly: empty inputs exit 0 and write a zero-byte `-o` file, so the merge → record chain works unchanged on a clean round (no manual file creation). The payload's `inputs` array reports `{path, parsed, skipped}` per input file, and an input with content lines of which **none** parsed exits **1** (v0.35.0) — that is a role whose whole file was mis-emitted, not a clean round, and recording it would freeze an undercounted round.
   ...
   - **Always `-o`, never `>`, and `&&` rather than `;` between merge and record.** A shell redirect creates the file *before* the process starts, so a merge that then exits non-zero leaves a zero-byte file behind and a `;` walks straight into `--record` with it — which records a **clean** round, because a zero-byte input is exactly how a converged round reports itself. Both `tp review --merge -o` and `tp audit --merge -o` decline the write when an input parsed nothing, so a refused merge creates no file at that path (and leaves a file already there byte-identical) and `--record` exits 3 on the missing one. So write it as `tp review --merge review-<base>-r<N>-*.ndjson -o merged.ndjson && tp review <spec> --record merged.ndjson` — name the base and the round, since a bare `r*.ndjson` in a directory holding two specs also sweeps the other spec's role files.
...
3. `tp import tasks.json` — **plain, no `--force`.** The init shell holds zero tasks, so the overwrite needs no `--force` and the §9.1 convergence checks stay armed: a review loop that is not done blocks the import with exit 1, and so does a registered review check that fails or cannot run, named in the refusal (`--force` skips both). A loop the cap ended passes without `--force`, and the payload says so (`done_by`, `fixed_at_cap`, `stale_waived`). Reserve `--force` for overwriting a file that already has real tasks — and only with explicit user approval.
...
4. Fix the code for every non-PASS item, then record how each was closed in the recorded round file `--status` names (`audit_rounds[].file`): `tp audit <round file> --resolve <0-based index|role:item_id> <fixed|wontfix|duplicate> "<evidence>"`. A `role:item_id` selector disposes every non-PASS row under the key and reports how many as `disposed`; `--resolve-all` disposes every undisposed non-PASS row and skips PASS rows; `--force` overwrites a disposition already there. A resolve into a file no round lists changes nothing and says `recorded_round: false`. **An evidenced `wontfix` or `duplicate` clears its round**, live on `--status` and `--check`; `fixed` does not — the next round re-reads the repair. The acceptance lasts: the next prompt's Prior Round block shows each row's disposition, and the next `--record` carries it onto the same `(role, item_id)` non-PASS row while the row's `evidence_file` is unchanged (reported as `carried`; `fixed` is never carried). A `wontfix`/`duplicate` with blank evidence exits 2. Accepting an audit finding without a code change is the operator's decision: under `TP_UNATTENDED` it exits 2 and escalates as `accept-finding`, while `fixed` stays unfenced.
...
per-role files (`audit-<base>-r<N>-<role>.ndjson`) a round may sit beside, which would double-count.
...
for each blocker: agent-clearable (unexplained-changes) → reconcile (commit or tp keep), re-run resume
                  escalate (no-ready-task, *-budget-exhausted, spec-stale) → stop, hand to a human
                    (next_action.summary names the blocker; command and brief_command are null)
...
| `tp next --peek` | Preview the next ready task without claiming; unlike plain `tp next` it ignores WIP |
...
| `tp review spec.md --round N --findings file.ndjson` | Multi-round with previous findings exclusion. A passed `--round` exits 2 under `--record`, `--status`, `--merge`, `--resolve`, `--resolve-all`, `--report`, `--verify` and `--perspective documentation`/`testing`/`regression` |
...
| `tp review spec.md --spec-inline` | Embed the spec inline, cut at 10,000 bytes and named in `spec_truncated` when cut (default is reference mode) |
...
- Once registered, tp runs the check every review round, reports it in `mechanical_checks`, and tells reviewers to stop reporting that class. A check's exit code is its contract: **0** passed, **1** found violations, anything else — 2 or higher, 126/127, a signal, a start failure, a timeout — could not run and is reported `ran: false`. A prompt suppresses a class only when its check reported `ran: true` in that emission, so a check that could not run — or never started, because no task file resolves — suppresses nothing. `tp review --status --check` requires every check to pass before exiting 0.
- **Registration retires the suggestion (v0.33.0):** a registered check retires its mechanize candidate — the class is dropped from `mechanize_candidates`, from the register-a-check `hint`, and from `next_action`'s mechanize branch, and `tp review <spec> --record <file>` names the withheld classes in `mechanized_classes`. Under `--record` and `--status --check` only a check that ran (exit 0 or 1) retires its class; plain `--status`, which runs no check, keeps registration as the trigger. `--record` runs the checks only when their result can change its output, and then carries `mechanical_checks`. A failing check still blocks `tp review --status --check`. Class matching is byte-for-byte.
...
Emission is corpus-driven: `tp review` emits one prompt per active reviewer role plus the built-in `regression` role (appended, never a corpus file); `tp audit` emits one per active auditor role. Every prompt stamps a five-key output contract: `role, location, class, severity`, plus `evidence` on a review prompt (v1.1.0) and `status` on an audit prompt. tp still only emits prompts — it never executes agents.
...
| The name is… | Outcome |
|---|---|
| in the emitted set | exit **0**, exactly one `prompts[]` entry, byte-identical to that role's entry in the same invocation without the flag; `skipped_roles` names every other emitted role with reason `role-filter` and, when non-empty, survives `--compact` |
...
3. **A `mechanize_candidates` class recurs, none blocking** (review only) → register a check (`tp set --workflow checks='[…]'`), then run the next round. The directive carries the phase qualifier: a check is **only worth registering when the artifact it measures already exists in the review phase**, because registered checks run in that phase alone. The un-mechanizable `over-specification` class never triggers this — it falls through to step 4. A class whose registered check ran falls through too (under plain `--status`, any registered class), so the driver is never told to write a check that already exists; a class whose check could not run is named with that check to fix.
...
**Registered checks override the review import steps.** Under `--record` and `--status --check`, at
done or at the cap, a registered check that failed or could not run replaces every step that names
import with that check to fix, ending `tp review <spec> --status --check exits 1 until every
registered check passes`. Plain `--status` runs no check, so while any check is registered its import
step starts `run tp review <spec> --status --check (--status alone runs no check) and on exit 0`.
