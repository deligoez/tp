# tp v0.53.0 — The spec-hash reset
...
copying it — this section's own predecessor cited `audit_record.go:101` and `:339`, and those are now
`:130` and `:392`:
...
`engine/resume.go` twice, and `review_status.go`.

**`engine/resume.go`'s audit line is the one that gets missed.** It feeds `DetectPhase`'s `release`
...
| 3 | §3 *resume* | `tp resume` reports the audit phase, **not** `phase: release`, in that fixture | **move the `internal/cli` sites and leave `engine/resume.go`'s audit line on the shared function** — every assertion three earlier drafts proposed still passes, and this is the section's own stated failure |
