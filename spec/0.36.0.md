...
**What it costs: 467 bytes per role prompt.** The arithmetic is written out because §6.2 test 5
...
- **A name the round does not emit but tp recognises is not an error.** It exits **0** with
  ...
  This split is forced by the driver, not chosen for taste. §4.2.2's brief change makes `--role <id>`
...
#### 4.2.2 The brief that passes it
...
the skip table above shows it is not, which is why the exit-0 rule exists and why §6.4 test 13
...
bytes to read 12,376 of them; with the brief of §4.2.2 it receives only its own. Across the round that
...
What this release does change is that the loss stops being silent. With §4.2.2's brief every unit
receives exactly the roles it was assigned, so a role nobody is assigned is visibly emitted to
nobody rather than buried inside a payload every unit happens to receive. §6.4 test 15 pins that
...
`--role` answers the same measurement without any of the four — but only with §4.2.2's brief change,
...
6. **Giving the `regression` role a unit of its own.** §4.1 measures that its prompt reaches every
   unit and is executed by none; §4.4 explains why the repair is new unit-lifecycle surface. Like
   Non-Goal 5 it is unwritten rather than assigned — `spec/0.38.0.md` is the backlog release and so
   the natural home, but it does not contain the section today and this spec does not pretend it
   does. This release makes the loss visible, not fixed.
...
### 6.1 There is no cross-version baseline
...
draft used ("in-process", and a separate binary in §7) described different harnesses for the same
number.

The comparison does not need a second version. The clauses are appended by one function, so the
baseline is **the same tree with that append skipped**, taken from the emitter directly rather than
from a release tag. Concretely: the test calls the prompt builder twice, once with the append
enabled and once without, and asserts the difference. That gives an exact, reproducible delta with
no tag checkout, no stored blob and no second toolchain.

Nothing is committed as a fixture either way: the payload embeds the round number, the loop budget,
the consecutive-clean count and the `output_path`, so a stored blob would encode this repository's
review state on the day it was written and fail on the next recorded round. Those four inputs are
fixed explicitly by the test's own temporary state directory — not by a comment saying they are.
...
7. `--role regression` emits one prompt at a round where regression is emitted, proving the match
   ...
   `skipped_roles` with that reason — not exit 2. This is the rule §4.2 states, and getting it wrong
...
8. A name tp recognises nowhere exits 2 with a hint listing the names the invocation would have
   emitted.
9. Each of the five skip reasons in §4.2's table is exercised: `--role <name>` for a role skipped
   ...
   today.
10. `--role` with each refusing mode in §4.2.1's table exits 2 with a hint naming both flags — run per
...
12. Without `--role`, the payload differs from the same tree's pre-clause emission only by the
    467-byte suffix inside each `prompts[].prompt` — §6.1's construction, not a cross-version
    comparison.
...
13. `tp resume`'s `next_units[].brief_command` for a `review-role` unit is
    ...
    `tp run --dry-run`.
14. **Every unit's own brief runs.** For each unit `BuildNextUnits` returns, execute its
    `brief_command` and assert exit 0 — under a fixture where a corpus role is skipped
    `no-checklist-items`, so the divergence §4.2's table names is present rather than hypothetical.
...
15. The emitted role set minus the unit set equals exactly `{regression}` on a round where
...
**It is a self-comparison, not a historical replay.** A draft gated the release on re-emitting each
recorded round from its `snapshot-round-R.md`. Measured across `spec/.tp-review/`: all 145 recorded
review rounds have a snapshot, but for **35 of them (24%)** the snapshot's sha256 does not equal the
`spec_hash` that round recorded — tp refreshes the snapshot when the spec changes, so snapshots
drift forward and a quarter of the corpus is not the text its round reviewed. A gate reading them as
history would compare against the wrong spec and report a clean pass. That draft also required a
`class` → checklist-item mapping, which exists nowhere in tp (`class` reaches a reviewer as an
optional free-text slug), and 303 of the corpus's 2,802 recorded rows (10.8%) carry no `role` or
`class` at all. All three defects come from gating on data that was never written for this purpose.

**What is checked instead**, per spec that has a role corpus, for `tp review` **and** `tp audit`,
against the current tree only:

1. **`--role` loses no bytes.** For each role the unrestricted invocation emits, the `--role <name>`
   prompt for that role is byte-identical to that role's entry in the unrestricted payload.
2. **`--role` loses no role.** The union of the per-role invocations' `prompts[]` equals the
   unrestricted `prompts[]`, as an ordered sequence. This is the assertion that actually catches
   suppression: a role that stops being emitted, or an entry that changes when selected alone, fails
   it — and it needs no history, no class mapping and no stored fixture.
3. **The clauses append and displace nothing.** The v0.36.0 body equals the pre-clause body with its
   trailing newline removed plus §2.3's 467-byte suffix, and nothing else moves. §6.2 tests 3 and 5
   are this criterion at the unit level; the gate runs it across every spec in the repository.

**Cost, because §1 prices the clauses and a draft left this unpriced.** One `tp review` and one
`tp audit` per spec, plus one per emitted role: on this repository's 15 specs that is roughly 150
emissions, seconds of CPU, no network and no agent. It is a test, not a cycle.

**Failing the gate blocks the release.** A spec with no role corpus is named as out of scope rather
