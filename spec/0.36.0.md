...
the experiment the motivating story describes cannot be run on it; and `spec/0.49.0.md` §9 has
...
no verdict for "asserted, not run". That gap is `spec/0.49.0.md`'s subject and Non-Goal 2 names it
...
one.** Three drafts named an owner without checking, and the third is the one worth recording: the
first two named the wrong file, and the third named the right file and was made wrong *underneath
it* by a renumbering. `spec/0.48.0.md` §2 is "Sharding a role's checklist" and §3 is "Incremental
rounds" — neither states a completeness predicate; `spec/0.38.0.md` was named next and states none
either. Both were re-read for this rather than re-counted, deliberately: an earlier draft supported
the same conclusion with a word-occurrence count and a section enumeration, and by the time a later
round measured them **both supports had rotted while the conclusion they supported had not**.
...
Sharding (`spec/0.48.0.md`) is the release that will be unable to proceed without it, because its N
shards are jointly complete and no single one is. Naming a file that does not contain the question is worse
than naming none — it converts an open problem into a closed-looking reference, and a renumbering
makes that failure silent, because the stale name still resolves to a file.
...
- **A name the round does not emit but tp recognises is not an error.** It exits **0** with
  `prompts: []`. Only a name tp recognises nowhere — absent from the corpus for either phase, from
  `skipped_roles`, and from the built-ins — exits **2**, with a hint listing what the invocation
  would have emitted.

  **Whether that exit-0 carries a reason depends on which way it was recognised**, and an earlier
  draft said "the role's own entry present in `skipped_roles`" flatly, which audit round 3 measured
  false for one of the two ways. A role *this phase* skipped keeps its own `skipped_roles` entry with
  the reason tp already computes. A role recognised only through the **other** phase's corpus is in
  no entry at all — `tp review <spec> --role spec-coverage` exits 0 and the array names whatever else
  the round skipped, never `spec-coverage`, because this phase did not skip it; it was never a
  candidate here. Both are exit 0 with an empty `prompts[]`; only the first is explained.

  **`--compact` is the one place the reason outranks §8.4.** That section omits `skipped_roles` as
  explanatory, and for a payload with prompts in it that is right. For an empty `--role` payload it
  is not: audit round 3 measured `--compact --role <recognised-but-skipped>` returning
  `{"prompts": [], "skipped_roles": null}` with an empty `instruction` and an empty stderr — zero
  bytes saying why — while the same call with a *typo* was diagnosed in full. So the correct
  invocation was the only one left in the dark. When `--role` empties the payload, the reason stops
  being commentary on the payload and becomes the payload, which is §8.4's own criterion for
  surviving `--compact`.
  ...
  **What a unit with no prompt does is stated as a consequence, not as an instruction.** Three
  drafts asserted that such a unit "still creates its findings file, empty" and none named a channel
  that could tell it so — the emitted prompt is the only carrier this release has, and a unit with
  no prompt receives nothing by definition. So the honest statement is what happens:
  `hooks/stop-role-incomplete.sh` fires for every `*-role` unit and applies `spec/0.35.0.md` §3.3's
  predicate to `$TP_ROUND_DIR/role-$TP_UNIT_ID.ndjson.part`; a unit that wrote nothing is refused its
  first stop, the hook stands down on `stop_hook_active`, and the unit exits on the second. That is
  one wasted round-trip and a confusing message — bounded, documented, and not repaired here.
  Removing it needs a channel that reaches a unit holding no prompt, which is new surface and not
  this release's.
...
**The seam is one line, and two drafts described it wrongly.** The first named
`UnitKind.BriefCommand` as the place; the second said that function "cannot see the id". Measured
while implementing: it can. `UnitTarget` carries an `ID` field and `BriefCommand` already takes a
`UnitTarget` — what was missing is that `newNextUnit` received the id *alongside* the target and
never set it on the target. Setting it there is the whole change.

The correction matters beyond the wording. "Cannot see it" would have sent an implementer to change
a signature; "is not set" sends them to one assignment. A spec that names a mechanism owes the same
verification as one that names a measurement, and neither draft ran the code before writing the
sentence.
...
before or together with the three role prompts."* Under `--role` the payload holds one prompt and no
regression prompt, so the instruction directs actions that payload cannot support.

**The rule turns on the payload, not on who is calling.** A draft grounded it on the caller being
"a unit, not the orchestrator", which §4.3 falsifies in the same release — a human orchestrator
invokes `--role` once per role, and nothing in the flag distinguishes the two. What is decidable is
what the payload contains. A draft changed what a unit receives and left this key
...
P1 asks whether a change serves the agent. Under `tp run` a role unit receives the whole panel's
payload to read one prompt out of it; with the brief of §4.2.3 it receives only its own.

Measured on this repository, comparing like with like — payload bytes against payload bytes, on a
**four-role panel with no recorded state**, which is the panel size §4.1 warns is round-dependent:
from round 2 `tp review` also appends `regression`, and that prompt alone is the largest in the
payload, so the review row moves with the round. The audit row's panel is four at every round, but
its bytes move with the **affected-file set**, which this table does not name.

**The ratios are what reproduces; the byte columns are stamps on one measurement.** Round 7
re-derived the table from scratch and got 65.7% and 61.3% against the 65.8% and 61.5% below, with
both byte columns inside 2% — so the table holds under the conditions named above it. Change one of
them and it moves: the audit ratio spans 55.3%–64.5% over the affected-file count alone and drops a
further five points with a large `--findings`, and the stated 61.5% is reached at roughly 150 files,
outside the 1–72 range a real round uses.

Two earlier drafts failed here in opposite directions and both are worth keeping out. One gave the
review figure with no round attached, repeating at small scale the error §4.1 exists to name. Its
replacement said "no absolute byte count is quoted" two lines above a table quoting four. Pinning
the audit ratio properly is a script rather than a fourth qualifier, and `spec/0.43.0.md` is where
measurement work belongs.

| | unrestricted × N | with `--role` | reduction |
|---|---|---|---|
| `tp review` | 144,348 | 49,349 | **65.8%** |
| `tp audit` | 306,068 | 117,773 | **61.5%** |

An earlier draft claimed **90.6%** by dividing *prompt* bytes into *payload* bytes. Audit round 3
caught it: a per-role payload cannot be the size of its prompt, because §6.2 property 4 requires
every other top-level key to survive `--role` — so each unit still pays for the checklist, the
structured elements and the loop block. That remainder is what separates the two figures, and it is
paid N times either way. The saving is real and large; it is not the one the arithmetic claimed.
...
1. Its only P1 claim — that under `tp run` "the spawner reads `prompt_path` and passes a file rather
   ...
   `TP_UNIT_ID` and seven others, and **no prompt at all** — neither a body nor a path. The unit
...
1. **Sharding a role's checklist.** `spec/0.48.0.md`. This release does not claim to be a
   precondition for it: sharding is more entries in a `prompts[]` array that already carries N of
   them, and that file names `spec/0.43.0.md`'s measurement as its real precondition.
2. **Provisioning an auditor with a way to perturb, or grading the evidence it returns.** §2's
   ...
   asserted finding, and `spec/0.49.0.md` owns both the recipe (§9) and the verdict for un-run
   evidence (§5). §2.4 states the gap rather than closing it.
...
7. **Changing what a round asks.** The checklist items, role rules and output schema are untouched.
   Two things do change and are named rather than hidden under "nothing": the two clauses §2 and §3
   append, and `review_loop.instruction` under `--role` (§4.2.3.1).
...
`spec/.tp-review/` and `.tp/rounds/` are byte-identical after the suite runs, and no emission the
suite makes may see a panel that shifted mid-run — properties 7 and 8 compare emissions across
separate invocations, and a round advancing or `regression` starting to be appended between them
would make those comparisons pass or fail for reasons that have nothing to do with `--role`.

**Only the first half has a mechanism, and audit round 5 named the gap rather than letting it
stand.** A before/after hash compares endpoints: a test that mutated the directories mid-run and
restored them would satisfy it exactly. The second half holds today by construction and not by
measurement — every emitting test relocates its spec, so nothing writes there at all — which is a
property of the current call sites rather than something the guard establishes. An earlier draft
called the second half load-bearing and pointed at the hash as its guard; the hash cannot see it.
The tests
...
it was written and fail on the next recorded round. All four are read from the emission at run time
instead — an earlier draft said they were "pinned by the test", which names no test and which round
5 could not attach to one. What S2 forbids is a stored blob; what replaces it is derivation, not
pinning.
...
2. No prompt whose `output_path` is empty **ends with** property 1's suffix. Positional, like
   ...
   a list written here.
3. **The suffix is appended once, not twice, and the strip happened.** For an emitted prompt whose
   `output_path` is non-empty, the body ends with property 1's suffix; that same body with its last
   468 bytes removed does **not** also end with it, and does **not** end with a newline. The second
   clause is what makes the strip observable at all — property 1's "ends with" cannot see a byte
   removed before the suffix, so without it no property in this release fails on an implementation
   that appends without stripping, the one byte §1.1 calls not optional. Decidable on any
...
4. **`<name>` falls in exactly one of three classes**, and properties 4 and 5 cover them without
   ...
   "Recognises" is §4.2.1's term, defined there and only there. A draft restated it here and the
   two statements disagreed on names this spec itself uses; a second definition of the word an exit
   code turns on is worse than none.
...
10. Each role unit's `brief_command` names that unit's own id, and running it exits 0 **in the
    phase that unit belongs to** — including where the unit set and the emitted set diverge, which
    §4.2.1's table shows they do. The phase qualifier is not decoration: §7 measures that
    `tp audit <spec>` exits 4 on a clean tree, and an audit-role unit runs after implementation,
    when the tree is not clean. A draft omitted the qualifier and §7's own measurement falsified
    this property in the same document.
...
**The `tp audit` half must be given its affected files.** Measured in a clean git repository,
`tp audit <spec>` exits **4** with `no changed files detected (staged+unstaged is empty)` — it does
not pass vacuously, it does not run at all, and a release tree is clean exactly when this gate
fires. The gate supplies them with `--affected-files` or `--base <previous tag>`; which one is the
implementer's, and a gate that names neither reports nothing for half its scope.
...
re-proposed. **Replaying recorded rounds does not work**: all 156 recorded review rounds under
`spec/.tp-review/` have a snapshot, but for 35 of them (22.4%) the snapshot's sha256 does not equal
...
corpus's 3,047 recorded rows (9.9%) carry no `role` or `class` at all.
