...
A later draft answered that by listing which section uses which figure, and the list was wrong at
half its entries the round it was written — a hand-maintained index of one's own document is a
second thing to drift. So there is no index: every use elsewhere writes "suffix" or "net delta"
next to the number, and a bare figure with neither word is the defect.
...
12. Without `--role`, the payload differs from the same tree's pre-clause emission only by §2.3's
    construction — trailing newline removed, then the 468-byte suffix — inside each `prompts[].prompt` — §6.1's property, not a cross-version
...
19. **`review_loop.instruction` matches what the caller holds.** The assertion is **derived from the
    two emissions, not from a written list**: split both instructions into sentences, and require
    that every sentence present without `--role` and absent with it directs an action a
    single-prompt caller cannot take, while every sentence kept is one it can. A draft enumerated
    the forbidden directives instead; measured on this round's emission the list was short by one
    and had to be re-checked by hand each time the key changed — which is the list-form
    specification §4.2.3 rejects, reappearing in the test that pins it. Without `--role`, the key
    equals the same tree's pre-change emission for the same spec, corpus and state directory, per
    §6.1. Both directions are asserted, because only the pair distinguishes "addressed the single
    prompt" from "stopped emitting the key".
