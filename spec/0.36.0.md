...
the experiment the motivating story describes cannot be run on it; and the evidence-contract work
(`spec/candidates.md`) has already assessed exactly this recipe and found against it, noting that N auditors each building a
...
no verdict for "asserted, not run". That gap is the evidence contract's subject and Non-Goal 2 names it
...
it* by a renumbering — **and a fourth renumbering has since falsified the two filenames this
paragraph itself carried**, which is the rule demonstrating itself. The sharding release's own
sections were "Sharding a role's checklist" and "Incremental rounds" — neither states a completeness
predicate; the release named next states none
...
Sharding is the release that will be unable to proceed without it, because its N
...
the audit ratio properly is a script rather than a fourth qualifier, and the repair-locality release
is where measurement work belongs.
...
1. **Sharding a role's checklist.** This release does not claim to be a
   precondition for it: sharding is more entries in a `prompts[]` array that already carries N of
   them, and it names the repair-locality measurement as its real precondition.
2. **Provisioning an auditor with a way to perturb, or grading the evidence it returns.** §2's
   ...
   asserted finding, and the evidence contract owns both the recipe and the verdict for un-run
