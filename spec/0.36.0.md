...
change to the body is 467, because the strip removes one. §6 and §7 name whichever applies and never
substitute one for the other — an earlier draft called the suffix "467-byte", which is an off-by-one
...
**What must hold**, for every spec the repository can emit prompts for, for `tp review` **and**
`tp audit`:
...
2. **`--role` loses no role.** The per-role invocations' entries, concatenated in the unrestricted
   payload's own order, equal the unrestricted `prompts[]`. Order is asserted against that payload
   rather than against the sequence the invocations were made in, which a caller controls and which
   would make the criterion satisfiable by construction.
3. **The clauses append and displace nothing.** The body equals the pre-clause body with its
   trailing newline removed, plus §1's **468-byte** suffix — the suffix figure, not the 467 net
   delta; §1's table distinguishes them.

**The gate must not write the state it measures.** `tp review <spec>` takes the round's snapshot as
a side effect — §2.2 records that this spec's own author tripped over it — so ~150 emissions against
the live tree would rewrite `spec/.tp-review/` while claiming to be a read-only measurement, and
would advance rounds it does not own. The gate therefore runs where those writes are inert. Whether
that is `--no-state`, a copied tree or a temporary state directory is the implementer's choice; what
is fixed is the outcome: **`spec/.tp-review/` and `.tp/rounds/` are byte-identical before and after
the gate runs**, and that equality is itself asserted.

**Cost.** Roughly 150 emissions on this repository — seconds of CPU, no network, no agent — plus
whatever the isolation above costs. It is a test, not a cycle. Registered `workflow.checks` run on
each emission, so a repository with expensive checks pays them 150 times; tp's own `checks` list is
empty, and a repository whose is not should scope the gate to the specs it is releasing.

**Failing the gate blocks the release.** A spec that emits no prompts at all is named as out of
