...
so every later use names which of the two it means — §2.3 and §6.2 test 5 the net delta, §6.1 and
§7 the suffix. This table is the only derivation; the figures appear elsewhere only as labelled
references to it.
...
off. The isolation must therefore preserve state behaviour and only relocate it. There is exactly
one way to do that: `engine.ReviewStateDir` derives the state directory from the spec's own path
(`<spec-dir>/.tp-review/<spec-base>`) and no flag or environment variable overrides it, so the suite
copies the spec — and any `.tp-review/` it needs — into a temporary tree and runs there. A draft
offered "a temporary state directory" as an alternative; tp has no such thing. The outcome is what
is fixed:
...
18. **The suite writes no review state.** `spec/.tp-review/` and `.tp/rounds/` under the repository
    ...
    per file, so a file the suite creates fails it as loudly as a file it edits. Because a member of
    a suite cannot observe the suite it belongs to, this runs as a `TestMain` bracket — hash before
    the tests, hash after, fail the package — rather than as an ordinary test that happens to run
    last. It is §6.1's first condition, owned by a named mechanism rather than left as prose.
19. **`review_loop.instruction` matches what the caller holds.** With `--role`, the key names no
    action the single-prompt caller cannot take: the assertion is a list of forbidden directives —
    spawning a sub-agent, processing the regression prompt first, merging, recording the round, and
    running a `--perspective regression` delta pass — none of which may appear. Without `--role`,
    the key equals the same tree's pre-change emission for the same spec, corpus and state
    directory, per §6.1; no release tag is involved. Both directions are asserted, because only the
    pair distinguishes "addressed the single prompt" from "stopped emitting the key".
