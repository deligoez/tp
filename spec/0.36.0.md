...
its bytes move with the **affected-file set**, which this table does not name either: round 5
measured 59.4% at one file, 60.7% at five and 62.1% at twelve, so 61.5% sits inside the range rather
than being wrong — unattributed in exactly the way the review row was.

The **ratios** are what reproduces. The absolute byte columns are stamps on a spec text that every
edit to this file moves; three independent re-measurements put the bytes within 0.3% (review) and
2.2% (audit) while the ratios held to 0.3 percentage points. An earlier draft gave the review figure with no round attached, repeating at small
