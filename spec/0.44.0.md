...
suffix. Developing `spec/0.38.0.md` with a binary built one minute ago from `HEAD` gives:
...
`v0.37.1 < 0.38.0`, so the advisory fires — on exactly the binary the rule asks for.
