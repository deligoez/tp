# v1.0.0 — measurements: the working map of the effort that shipped `tp ground`

**Formerly `spec/working-map.md`, renamed on 2026-09-08.** This is the cycle record of `v1.0.0`,
written while the effort was in flight and kept as history: the ground protocol's four runs, the
`v1.0.0` review loop's pre-registered decision rules and their outcomes, and the work-item table as it
stood when the effort finished. `spec/1.0.0.md` is the decisions; `spec/1.0.0-corrections.md` is the
claims that document withdrew; this file is how the cycle went. Recorded rounds under
`spec/.tp-review/1.0.0/` and two ground rounds under `spec/backlog/.tp-review/` cite the old name.

**This file is an index, not a store.** A finding lives in exactly one place: the spec that takes it.
The map gists it and points; it never restates it. Not a release; the version in its name is the
release whose cycle it records. The effort it maps finished: `tp ground` shipped as `v1.0.0`, the
hotfix it waited on became `spec/backlog/a-finding-can-leave-an-audit-round.md`, and the pending specs
were re-verified and reordered in `spec/backlog/README.md`.
