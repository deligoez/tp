...
**All ten now have a release, and writing those releases falsified four of the entries below.** Each
spec re-derived its own findings rather than inheriting them, which is the rule this file states, and
four inherited claims did not survive it. **The corrections are in the specs, with the measurement;
the rows below are left as written so the two can be read against each other** — an entry silently
edited to match its own correction teaches nobody why the first reading was wrong.

| this entry says | the spec that took it measured |
|---|---|
| a **fenced** decoy row inside §2.1 leaves both packages green | **closed, not open.** `require.Len(rows, 1)` counts the fenced row too, so the helper refuses rather than choosing. The real cost runs the other way: a fenced quotation of the arms table beside an otherwise *correct* spec is a **false failure**, on text §2.1 step 1 itself rules non-content |
| `groundSection72Fields` is *"the weaker of the two holes"* because its callers pin hard | **the same defect.** Adding `` \| `Bogus_field` \| string \| no \| … \| `` to §7.2's real table is silently green in both packages: the `[a-z][a-z_]*` first cell drops it and the count of thirteen still matches. "Fails loud" holds only for a field the parser can spell — a defect nobody had recorded |
| two helpers are in this class | **three.** `groundSection72VerdictRule` reads the same §7.2 with the same fence-blind bound, and surfaced only when the §7.2 fenced-heading probe failed it alongside its siblings |
| the U+000B failure direction is safe, because a mismatched hash drops the carry | **true of the hash sites, false of the step-1 sites.** A VT before an opening fence makes the code block the only floor unit and **silently drops the real claim** — index line `1 in floor, 0 cut`, byte-identical to the control's |
| `rankFilesBySpecTerms` goes to the checklist release, *"which already owns the file-selection channel"* | **wrong channel.** Its two call sites are both `internal/cli/review.go`, selecting docs and tests for the **review** phase; the checklist release reworks `selectCodeFiles` in `internal/engine/auditfiles.go`. A search for `SelectAuditFiles\|AuditFileInputs` over `internal/` returns three files and `review.go` is not among them |

**Four of the ten are sentences that are false as written, and they were left in the tree
...
sentence, and each is now owned by the release that fixes its subject:
...
**Where each of the ten now lives.** Named by subject, because a number that moves leaves stale
references and this repository has paid for that five times; the roadmap table in `CLAUDE.md` is the
one place the numbers belong.

| finding | the release that owns it |
|---|---|
| the guard re-parses Markdown production already parses (all five doors, all **three** helpers) | *the guard reads what production reads* — it takes the seam (`floorBlocks` + `floorAnchorsByLine`, prototyped and run), and both false sentences about these helpers |
| the enum-refusal guard is a lower bound; `tier`'s second refusal names no set; `rankFilesBySpecTerms` drops in silence | *the refusals that name nothing* — and it carries the routing correction above, plus the enum test's false spec-binding sentence |
| the empty-floor ask collapses two zeros; the envelope cannot separate them either | *the ask and the envelope separate the two zeros* — taken together, per the finding's author, with the ask's own false sentence |
| an unrepaired `FAIL` is not permanent in the deletion direction; §2.1 names no whitespace set | *what §8's carry can promise* — one release, because both are about how a unit's identity is computed and matched |
| the deferred finding that reached no lens | *the checklist covers what changed*, as its fifth decision — and the measurement there corrects `go-safety`'s own statement of it: the file never left the round's checklist, it left one role's |
...
**Taken.** It is now a release of its own, and writing it moved both figures below: re-derived at HEAD
the rule reports **2** violations rather than the *"0 today"* recorded here, both introduced by
`spec/1.0.0.md` after this section was written, and **48** rather than 18 at `v0.37.0`, because that
tree's specs carry pre-renumbering filenames. The four-direction table and the single false positive
did reproduce exactly. `scripts/forward-spec-ref-prototype.py` is the derivation; do not quote a
count from this section.
