...
**It needs the emit-time hash.** The measurement compares a round's snapshot against the previous
round's; that comparison means what it says only once a round's hash is pinned to the snapshot its
roles were given.
...
|---|---|---|---|---|
| v0.35.0 | 19 | 45% | 22% | 2.0× |
| v0.36.0 | 14 | **80%** | **22%** | **3.6×** |
| v0.37.0 | 11 | 92% | 71% | 1.3× |
| **all** | **44** | **70%** | **30%** | **2.3×** |

All figures are medians across the cycle's rounds.
...
against the changed sections by prefix, so `§3.2` counts as inside `§3`. Sub-section granularity
would need every finding to carry one, and 9.9% of recorded rows carry no `location` at all — those
are excluded from the numerator **and** the denominator, and the excluded count is reported.
