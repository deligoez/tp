...
**So the safety argument has to be built rather than cited — and the first attempt at building it was
withdrawn in the round after it was written.** That draft proposed rejecting an out-of-enum severity
at `--record` "the way an unknown `category` is", plus a fail-closed rule for unrecognised values.
Round 2 refuted it four ways and the section records them rather than patching around them:

1. **The cited precedent does the opposite.** `audit_record.go:282` returns early when
   `category == ""`, and `TestCountAuditFindings_AcceptsTheEnumAndAbsentCategory` pins that
   deliberately — *"treating that as invalid would reject every clean round."* An engineer following
   the analogy ships a validator that accepts exactly the missing case the sentence said must be
   rejected.
2. **The two rules cannot both be live.** If `--record` refuses the row, no future round can carry an
   unrecognised severity, so a fail-closed rule governs only rows already on disk. Rejection and
   *treat-as-blocking* are different tasks with different exit codes and different behaviour under
   `TP_UNATTENDED`; v0.31.0 §4.1 chose the second, and citing it for the first inverts it.
3. **It refuses fifteen of this repository's own recorded rounds.** The 67 out-of-enum rows sit in
   fifteen round files across v0.29.0–v0.32.0, and `countAuditFindings` aborts the whole file at
   `ExitValidation`. Under `tp run` the authoring subagents have exited by then, so the recovery is a
   round re-run.
4. **It does not address the failure it was introduced to answer.** §2.1's two misses are rounds that
   never run under `blocking` — v0.35.0's `error` row is in round 4 and `blocking` stops at round 3.
   **Validation makes a label well-formed, never truthful, and cannot make an unrun round run.**

**What the release needs instead, stated as the requirement rather than as a mechanism**, because
this section has now produced two mechanisms in two rounds and both were wrong within hours: before
`blocking` may gate, the severity a row carries must be *checkable by something other than the row's
author*, and the 67 recorded rows must have a stated disposition. Four incompatible answers to the
second exist and this section picks none of them, which is the honest state and is why §7 has no test
for it yet.
...
gated writes the label. **That objection does touch `severity`, and an earlier draft of this paragraph denied it in the same
words the paragraph above now retracts** — "an existing enum, validated at the schema, carried by
every row already recorded", every clause of which is false. The distinction that survives is
narrower and it is not about validation:

`scope` is a claim **about the finding's relationship to the release** — is this defect in scope —
which only the author knows and which no artifact can check. `severity` is a claim **about the
finding**, and it is checkable against the finding's own text by a later round, which is what every
round of this cycle has been doing to the round before it. That is a difference of degree, not of
kind, and it is the whole of the case. `scope` stays deferred because it is the further point on the
same axis, not because `severity` sits on the other side of a line.
...
would have converged that run at round 3–4 with the remaining rows surfaced — nothing hidden, and the
gate answering the question the operator was actually asking.

**Two corrections this sentence needed, both from round 2.** `nonblocking_open` is a **review-only**
field (`review_record.go:231`; `spec/0.31.0.md` §8.4 and `skills/tp/REFERENCE.md:634` say "never an
audit field"), so surfacing the remaining rows means creating an audit-side counterpart, which this
release must declare and test rather than assume — §7 does not. And the pointer that used to close
this paragraph named §5, which is now the removal notice; the `--check` half moved to
`spec/0.46.0.md` §8.

**That move left a requirement in no release, and this one depends on it.** §8 there records that the
same blocking/non-blocking distinction has to reach `--status --check`, or `--check` stays an
instrument nobody can ship on. Until it does, `audit_converge_on=blocking` changes what `--record`
reports and changes nothing about the command an operator or `tp run` actually gates on — so this
release either takes the `--check` half back or states plainly that it does not close the gap it
opens with. **It takes it back:** `--check` reads the same predicate as `--record`, and §7 gains the
...
**The 1–1 split this paragraph used to rest on is withdrawn, and the column that withdrew it is the
one this table did not have.** It asks *when the loop stops*; it never asked *what was still to be
found after it stopped*. That column reads `error` in both rows — v0.35.0's round 4 carries
`{error 1, warning 2, info 1}` one round after `blocking` would have halted at round 3 — so the score
is **2 of 2 cycles in which `blocking` would have shipped over an `error` row**, not 1 of 2. The
"repair artifact" defence above is unavailable at one round's distance.

So the honest reading is not that `blocking` is wrong but that **the two cycles agree against it on
the only question that matters, and one
...
**The mechanism as written cannot reach the case that motivates it, and round 2 measured why.**
`clean` is stamped at record time (`audit_record.go:71`, `:208`) and `ConsecutiveClean` walks the
stored flags, while `--resolve` writes the disposition into the round file *afterwards*. The next
round's merged file is built fresh from the role files, so the re-reported item arrives carrying no
`resolved` key at all. Excluding disposed rows "from the round's finding count" therefore excludes
nothing in the flow this section describes. Either audit's clean flag is recomputed live from rows,
the way review's twin already does (`review_status.go:91`), or the disposition has to survive into
the next round's input — and this release has to pick one, because §7 test 4's nine-round replay is
only meaningful under live recomputation.

**A second reader breaks it before the first does.** `dedupAuditRows` (`audit_merge.go:191-203`) keys
on `role\x00item_id` and keeps the **first** row, comparing nothing — where review's merge keeps the
highest severity. Under `blocking`, the order the role files are passed to `--merge` decides whether
a round is clean.
...
**Mechanism, and it costs less than "reset" suggests.** `consecutive_clean` is not stored: it is
derived at output time by `engine.ConsecutiveClean` walking `AuditRounds` (`audit_record.go:101`,
`:339`). So the change is to stop that walk at the first `spec_hash` boundary — zero stored bytes,
idempotent under `RecordRound`'s rewrite path, and retroactive over rounds already on disk, which
makes §7 test 7 true by construction rather than by fixture. The rounds themselves are not discarded
— a recorded round stays a frozen fact — only the streak they were feeding.

**One collision this creates.** `engine.ConsecutiveClean` is shared with review's `--status`
(`review_status.go:91`). Changing the walk changes both phases unless the reset is applied by the
audit caller, and Non-Goal 4 says this release does not touch review. The audit-caller form is the
one that composes.
...
3. **Changing severity's meaning or vocabulary.** `error | warning | info` is unchanged; this release
   only reads it. **This Non-Goal now binds:** the withdrawn sink-validation draft would have made
   this release *write* a rejection rule, and §2 says why that was wrong. Any future proposal that
   makes `--record` refuse a row is a different release, and this one must gate on what is already
   recorded or not gate at all.
