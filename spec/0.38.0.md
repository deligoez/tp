# tp v0.38.0 — The checklist covers what changed
...
sorted once, alphabetically, by `filterAuditUniverse`'s closing `sort.Strings` (`auditfiles.go:93`),
...
deleted}` per path (`auditfiles.go:56`) and `diffSummaryOf` already renders it into every emitted
...
**This makes the notice at `audit.go:777` true for the first time.** That notice already tells the
...
`SelectAuditFiles` already computes `CodeFilesTotal: len(universe)` (`auditfiles.go:79`), and
`buildRolePrompt` already renders `## Affected Files (10 of 46)` into the prompt
(`audit_roles.go:254-257`). **The role is told; the operator is not.**

The only truncation notice tp emits is `audit.go:768-777`, gated on `maxAutoDetectFiles` = 50
(`audit.go:100`). A universe under fifty passes that gate in silence while `CodeFileCap` drops
...
2. **No change to `spec-coverage`'s selection.** Measured on the same round-7 emission, its heading
   ...
   task-mapping count (`selectSpecCoverage`, `auditfiles.go:128-168`), which is the right key for a
   conformance lens.
