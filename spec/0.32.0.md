...
A refusal exits before any prompt is emitted and before any state is written. The §2.3 warnings travel `output.Notice` — stderr, suppressed by `--quiet`, **not** suppressed by JSON mode — so an agent driving tp with stdout piped does see that its entry matched no active role. Nothing here orders them against the error object.
