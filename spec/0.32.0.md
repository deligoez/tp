...
Domain filtering (§2.3), the unknown-id warning (§2.3), then the `spec-coverage` refusal, then the empty-phase refusal. The `spec-coverage` refusal precedes the empty-phase one because it names a single entry to remove. A refusal emits its own error object and nothing else — no prompts, no state, and none of the §2.3 warnings, which belong to a run that continues.
...
7. Deactivating every role of a phase exits 2 with the empty-phase message on stderr and an empty stdout, and leaves no `.tp-review/<spec>/`, no `state.json`, and no snapshot — asserted for `tp review` and `tp audit` separately. No §2.3 warning is emitted alongside it.
