...
This spec assumes the v0.31.2 routing, in which `spec-coverage` is the only auditor id that changes routing. `enabled: false` applies to it like any other corpus role; §2.5 is the only protection, and it fires only when the phase would empty.
...
Open means, per phase: for review, a finding in **any** recorded round that survives the resolved-away test `ReviewRoundClean` applies (`reviewFindingResolvedAway`, currently unexported in `internal/engine`; this release exports it or adds an exported wrapper, since the warning is emitted from `internal/cli`); for audit, a row in the **latest** recorded audit round whose `status` is not `PASS`. Attribution is best-effort — a recorded row carrying no `role` produces no warning.

The warning goes to the same channel as tp's other corpus messages and is therefore suppressed by `--quiet`. Like §2.4's `--compact` gap this is accepted: the deactivation is declared in the spec, and `--quiet` is opt-in.
...
Domain filtering (§2.3), the unknown-id warning (§2.3), then §2.5's refusal, then the §2.6 warning. §2.5 precedes §2.6 so that a refused run emits nothing at all (§6 item 7); only §2.5 stops the run.
...
8. The same spec, under every other mode of §2.5 clause 1, exits with that mode's own contract and never with the empty-phase error: `tp review --record` and `--merge` exit 0; `--status --check` exits 0 or 1 by convergence, never 2. Asserting the absence of the empty-phase error message — not merely a non-2 code — is what fails if the refusal leaks, since a malformed argument also exits 2.
...
11. Deactivating a role with a review finding in any recorded round surviving the resolved-away test warns with role, count and round file, and exits 0. A row with no `role` produces no warning; `--quiet` suppresses the warning.
...
4. `internal/cli/domain_lens_test.go` and `internal/engine/lens_shim_test.go` — both exercise `TranslateLegacyLens`, whose return type changes.
