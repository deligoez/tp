...
7. Deactivating every role of a phase exits 2 with the empty-phase message on stderr and an empty stdout. Asserted per command over that command's own artifacts: `tp review` writes none of `.tp-review/<spec>/`, `state.json` or the round snapshot; `tp audit` writes no round snapshot. The artifact sets differ because only `tp review` calls `EnsureReviewState`.
8. The buffering rule of §2.6 is tested on a fixture that can fail it: a spec that both names an unknown role id (producing a §2.3 "matches no active role" warning) and deactivates every role of the phase. The run exits 2 with the empty-phase message alone — the warning is discarded, not written. The same spec with one role left active flushes the warning and proceeds. Item 7's fixture cannot exercise this, since every id it names matches an active role and no §2.3 warning is produced at all.
9. The same spec, under the other modes that take it as the positional, never produces either §2.5 message: `tp review --record`, `tp audit --record` and `tp review --verify` exit 0; `--status --check` exits 0 or 1 by convergence. The assertion is the absence of the two literal messages, not a non-2 code, since a malformed argument also exits 2. `--merge`, `--resolve`, `--resolve-all` and `--report` take an NDJSON positional and never parse the spec; `tp review --perspective` takes the spec but short-circuits before the corpus is resolved. All five are unaffected by construction and are not asserted.
10. `regression` does not satisfy §2.5's emptiness check: a review spec deactivating every corpus reviewer still exits 2.
11. `tp.audit_roles` deactivating `spec-coverage` exits 2 with its own message even when other auditors remain active; `tp.review_roles` naming `spec-coverage` takes the "matches no active role" path instead. A spec tripping both refusals reports the `spec-coverage` one, per §2.6.
12. A spec deactivating every user role exits 2 rather than falling back to the embedded corpus — pinning §2.3's placement.
