...
| `review-budget-exhausted` | escalate | phase is `review`, and recorded review rounds reached a non-zero `review_max_rounds` without convergence (a `review_max_rounds` of 0 means no cap and never fires this blocker) | `{"cap": <int>}` |
| `audit-budget-exhausted` | escalate | phase is `audit`, and recorded audit rounds reached a non-zero `audit_max_rounds` without convergence (0 = no cap, never fires) | `{"cap": <int>}` |
