...
could parse"* to *"every backticked span in the row"*, which is the difference between rows 10 and 11
being silent and being red.

**Row, not cell — and no input on this document separates the two.** The pattern runs over the whole
row line, as the shipped one does. §2.1's arms row has `**verb**` as its first cell and that cell
carries no backticks, so the row's spans and the second cell's spans are the same set here and every
§2 row measures both readings at once. The requirement is stated over the **row** because that is
what is implemented; a document whose first cell carried a backticked span would tell them apart, and
this one does not.
...
- **A row whose first cell is a single backticked span is a field row, unless that span parses as one
  ...
  already uses it on the same rows, so the two guards partition one row set instead of cutting two
  windows. **Two conditions, not one**: the *shape* condition — a first cell that is a **single**
  backticked span — carries as much of the split as `ParseGroundVerdict` does, and the third bullet
  below is the row it decides on its own. What the seam removes is the second window, not the second
  condition.
...
| 3 | §4 | the verbs are every backticked span in the row | `` `([a-z-]+)` ``: measured **green** on §2's rows 10 and 11 — `Re-ran`, `re‑ran` (U+2011), `ran2` — and **red** under the seam |
...
| 5 | §3 item 3 | an absent §2.1 fails naming the anchor | delete `require.Contains(byLine, anchor)` from `specTableRowsUnder`. Fixture: `### 2.1 The floor` renumbered to `### 2.9`. With it, `does not contain "§2.1"` under *"spec/1.0.0.md must carry a §2.1 heading outside a fence"*; without it, `"[]" should have 1 item(s), but has 0` under a missing-table message — the wrong cause §3 item 3 exists to prevent. Both measured. **`HEAD`'s `slices.Index` locate is not this row's mutant**: on the same input it fails at `floor_test.go:1289` naming the anchor, so it satisfies the assertion |
| 6 | §4 | a real second unfenced verb row still reddens | drop the one-row requirement (`require.Len(rows, 1)` → `require.NotEmpty`). Fixture: **a second unfenced verb row carrying the twelve correct verbs, beside an otherwise-correct spec** — with the requirement, `should have 1 item(s), but has 2`; without it, `ok engine 0.236s`. Not §2's row 4, which is red both ways and cannot separate them — measured |
...
| 10 | §5 | a real 14th field still reddens | none needed: §2's row 14 is red before and after. The assertion is a **pair, in two tests** — `require.Len(inSpec, 13)` at `groundrow_test.go:588` and set equality with `groundRowKeys` at `groundrecord_test.go:306` — and row 14 fires both |
...
| 12 | §3 | the whole suite is green on the unmodified document | none: this is the control, and it is what makes rows **4, 7 and 11**'s green side mean *"the seam does not redden a document carrying no defect"* rather than *"this fixture happens to pass"*. **Three, not four** — an earlier draft listed row 6 as well, and row 6's only green is on the **mutant** side — the defect going silent — which is the direction this control says nothing about, and row 9's green side is this control run itself |
