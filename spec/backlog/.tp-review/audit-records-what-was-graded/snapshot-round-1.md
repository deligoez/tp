...
it was emitted in. The path is taken cleaned, so a leading `./` or a doubled separator does not make
a second id. The id keeps a readable prefix derived from the path and carries no positional suffix,
and no emission hands out one id for two different files. Rounds emitted under it carry a new
`id_scheme` value — the emission's scheme, not that of the binary that records the round.

**Decision 2.** `tp audit --merge` and `tp audit <spec> --record` **refuse** two rows with the same
`(role, item_id)` whose `evidence_file`, `status` or `severity` differ, instead of keeping the first.
The refusal has the shape `--record` already gives a row outside the category enum: exit 1, every
conflicting pair named by input and line at once, nothing written. Rows that agree on all three are
not refused; `--merge` still keeps one of them.

**Why both.** Decision 1 removes the known cause. Decision 2 makes every other cause loud — a role
that misnames an id, a hand-concatenated results file, one file named in two overlapping shards, a
residual collision in the derivation — and it ends a choice tp was never entitled to make silently:
which of two disagreeing verdicts on one item is the round's. Severity is part of the verdict because
under `audit_converge_on: blocking` it decides whether the round is clean.

**Consequences.**

- The prior-round section treats a round under an older `id_scheme` as not comparable, as it already
  treats a marker-less round; the two schemes' ids are not built to match. Recorded rounds keep their
  ids; tp does not rewrite them.
...
**Decision.** Spec-coverage's task-to-file mapping and `--affected-from-tasks` read the same shas for
a task: every `commit_shas` entry, or `commit_sha` when that is all a task carries. The
task-acceptance item's evidence names every file so mapped.
...
2. The re-record `next_step` is offered only when a driver round is set (`TP_ROUND`) and it is the
   recorded round the file's rows match, found as decision 3 finds it. Otherwise no resolve payload
   names `--record`.
3. A `--resolve` or `--resolve-all` into a file that is **not a recorded audit round** still writes,
   ...
   recorded round whose rows include every row of the file, dispositions aside — found by content
   across the project's recorded rounds, never through the active-file pointer, which can name another
   spec — or, when none does, that none does. A recorded round is a file its spec's round state lists
   as a round's file. **`tp review --resolve` says the same**, because the same write into the same
   kind of file is lost the same way on the review side. Whether such a write should be refused, and
   what a disposition in a recorded round does to its verdict, stay with
...
| 1b | §2 d1 *spelling* | one path spelled three ways — plain, with a leading `./`, with a doubled separator — each named alone with `--affected-files`, gets one id. At `HEAD`: one id | derive the id from the path as given, uncleaned |
| 1c | §2 d1 *uniqueness* | two paths constructed so that their derived ids coincide, emitted together: the checklist carries two distinct ids. *Deferred* — the pair depends on the derivation the task picks | emit the derivation unchecked |
| 2 | §2 d1 *prior round* | same fixture as row 1, round 1 recording a `FAIL` on the first file: in round 2's prompt, the prior-round row and the checklist item carrying the same id name the same file. At `HEAD` they name different files | restore the positional suffix |
...
| 4b | §2 d2 *severity* | `--merge` over two inputs sharing one `(role, item_id)`, `evidence_file` and `status: FAIL`, one `severity: warning` and one `severity: error`, exits 1 and names both lines. At `HEAD`: exit 0, `duplicates_removed` 1, the `warning` row kept | compare `evidence_file` and `status` only |
...
| 7 | §2 *scheme* | a prior round recorded under the older `id_scheme` value is rendered as not comparable. At `HEAD`: rendered as comparable | treat only a marker-less round as not comparable, the shipped predicate |
| 7b | §2 d1 *emitter* | a round emitted by a binary that predates this scheme and recorded by one that has it is rendered as not comparable in the next round's prompt. *Deferred* | stamp the recording binary's scheme |
...
| 12d | §4 d3 *one role's file* | a resolve into one role's findings file, whose rows are a subset of the recorded round's, names that recorded round. *Deferred* | match only a file whose rows equal a round's |
