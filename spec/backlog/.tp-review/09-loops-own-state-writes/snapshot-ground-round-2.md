...
> Read the rows it returns, not the count: the only *audit* atomicity row is
> `snapshot-tmp-race-under-parallel-role-units`, about `WriteSnapshotAtomic`'s fixed temp name and
> recorded fixed inside that same cycle, and the two review rows that match
> (`atomicity-mechanism-unspecified`, `partial-write-policy-unstated`) are both about the `--out-dir`
> prompt set. None of them is about the round findings file's write.
