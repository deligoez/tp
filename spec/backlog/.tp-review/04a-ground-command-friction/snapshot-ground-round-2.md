...
>
> This file takes `spec/candidates.md`'s **ground-related** material; the stub itself stays, for 29
> surviving references. That file held three kinds of thing, and `spec/undecided.md` took the larger
...
> tree cannot check, and §1.1 says which claims that covers. **Ten numbered items, fourteen declared
> decisions, and stating the counting rule is the point of the sentence.** Ten is §1's list plus
> §1.2's. Fourteen is `grep -c '^\*\*The decision' spec/1.0.1.md`, which returns **13**, plus §9.1's
> own heading; the anchored pattern matters, because a bare `grep -c 'The decision'` also counts this
> sentence and returns 16. The gap of four is §9 contributing three (§9.1's gate, §9.2's key, §9.3's
> ship-order call) against one item, and §10 contributing three (§10.1's `asked`, §10.2's
> `floor_delta`, §10.3's two halves) against one.
...
nobody enumerated.** At the commit that adds this file a round is recorded for **19 of the 22**:
`spec/1.57.0.md`, `spec/1.58.0.md` and `spec/1.0.1.md` itself have none, the first two having existed
for only the programme's last five and a half hours. Enumerate them rather than trusting the sentence
— `ls spec/.tp-review/*/ground-round-*.ndjson | cut -d/ -f3 | sort -u`. Counting rule for the figures
...
before it was written down. Two did not survive and are recorded with the measurement that refutes
...
The round is in the name; the spec is not, so both of these print `ground-r1.ndjson`:
...
un-qualified by spec, so two concurrently-reviewed specs collide exactly as §2's do — and that
interactive path is the one the grounding programme used. Under `tp run` it does not collide:
...
**8 such units across 2 specs, out of 1,990 floor units scanned — 0.40%.** They are `u84 u87 u89 u91`
in `spec/1.41.0.md` and `u71 u73 u93 u100` in **`spec/1.0.0.md`**, the document that defines the
floor. **The set is enumerated because an earlier draft's phrase was not.** That draft said *"every
pending spec plus `spec/1.0.0.md`"* and quoted 1,968, and **no reading of that phrase returns it**:
1,990 for the twenty-one above, 2,064 adding `spec/0.37.1.md`, 2,275 adding `spec/1.0.1.md` too. The
eight ids and the two filenames reproduce exactly under every reading; only the denominator moved,
which is the half a stated counting rule exists to pin.
...
first, by §5, which was in the file from the start and whose gloss nobody re-read. All three were
true of the four-defect release this file opened as; a non-goal quietly dropped, or left standing
with a reason a later section refutes, is the shape §5 and §14 exist to refuse.
...
1. **No change to the floor's *arms*.** The arms are §2.2's cut step — `internal/cli/ground.go` prints
   *"§2.1 produced %d units and the arms cut every one"* and `groundcarry.go` has *"The absence of the
   ...
   question"*, was broader than the non-goal and §5 falsifies it — §5's decision removes 8 units from
   the floor, by the counting rule §5 states. What it removes is a two-byte list marker, a *fragment*
   and not a sentence, so the non-goal stands under tp's own vocabulary and only its gloss was wrong.
...
| 8 | §5 | a document with a `N. **Bold.** text` list produces no floor unit whose whole text matches `^[0-9]+[.)]$`, **and** produces the same number of other units as before | drop every fragment shorter than **10** bytes, which also drops `Measured:` — the 9-byte legitimate unit §5 names. That is the only length floor this row kills, and it is the least tempting one: probed over 527 real floor units, markers are 2 bytes and `Measured:` is 9, so every threshold in §5's own blessed 3–8 range survives this row. Row 9 is what kills those |
| 9 | §5 *not a length rule* | a legitimate **2-byte** unit that is not a marker (`Go`) survives | implement §5 as a length floor of any kind. Two bytes is the marker's own length, so no threshold that drops the markers can keep this unit under either reading of the comparison. An earlier draft asserted a **3**-byte unit and does not kill them: under *drop when length < T* — the reading §5's 3–8 range makes natural — **T = 3 satisfies rows 8 and 9 together**, dropping every 2-byte marker, keeping `Measured:` and keeping the 3-byte unit. That is a length floor, which is the rule §5 argues hardest against, and it survived the table until this round |
