...
3.2 **Round-trip preservation.** Every command that rewrites the task file through `WriteTaskFile` — currently `tp add`, `tp claim`, `tp next`, `tp set`, `tp done`, `tp close`, `tp reopen`, `tp remove`, `tp commit`, and `tp import` — leaves the set of present `workflow` fields unchanged, **except for the single field a `tp set --workflow <field>=<value>` intentionally adds or updates** (§6.1). The `WriteTaskFile` call site is the normative boundary: any command that rewrites the file is bound by this invariant, and the list above is its current membership. A `"workflow": {}` block stays `{}`; a block with only `review_max_rounds: 4` keeps exactly that one field; a source file that arrives with no `workflow` key at all is normalized to `"workflow": {}` on the next write (§4.4), never populated with defaults.
...
6.1 `tp set --workflow <field>=<value>` adds or updates exactly that field in the task file's `workflow` block, leaving every other field's presence untouched (it never materializes siblings). The value is validated **before** the file is written against the field's range — `review_clean_rounds` and `audit_clean_rounds` are 1–10, `gate_timeout_seconds` is 30–3600, and `review_max_rounds` and `audit_max_rounds` are 0–50 (0 = no cap, a valid boundary) — so an out-of-range value (e.g. `review_clean_rounds=0` or `audit_max_rounds=51`) is rejected with exit code 1 and an actionable hint, and the task file is left unmodified. Unsetting a field is out of scope for this release (§9).
