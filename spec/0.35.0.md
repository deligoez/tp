...
file instead of skipping it. When every role in the panel satisfies that predicate and the round has
no recorded entry, the oracle returns the single `review-record` or `audit-record` unit for that
round: the step between collecting a round and acting on it, and the one point at which `next_units`
would otherwise empty — stopping the run with `no-units` — while the round's own work is unfinished.
...
One disposition is outside that guarantee, and the boundary is the shipped recorder's, not this
release's: `tp review --record` **refuses** a row pre-resolved `fixed` and exits 1, because a fix
means the spec changed and the round's findings were read against older text. So `wontfix` and
`duplicate` re-record unchanged, while a round that has accumulated a `fixed` disposition is not
re-recordable at all — its correct successor is a new round, which is what the recorder's own hint
says. The retry window this section promises therefore ends at the first `fixed`, and a driver that
reaches that state has a round to run, not a unit to retry.
