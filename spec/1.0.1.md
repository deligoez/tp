...
disposing some but not all of a round's findings does not clear it — every non-`PASS` row must carry a
disposition, so accepting the blocking ones and leaving a non-blocking one open still reports the round
not clean. The exit is `--resolve-all` with a shared
