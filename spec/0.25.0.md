...
3.6 `tp lint` validates the role corpus whenever a `.tp/reviewers/` or `.tp/auditors/` directory exists. Each file must: parse as JSON; contain all required fields (§3.3); have `id` equal to the filename stem, matching the kebab-case pattern, and not equal to the reserved `regression`; carry `domains` as a string array if present; and contain no unknown top-level key. A malformed or invalid role file aborts a role-reading command **for the phase it reads**: `tp review` on a bad **reviewer**, `tp audit` on a bad **auditor** (a broken auditor never blocks review, preserving phase independence), and `tp lint` on either — each with exit 3 and a `repair or delete <path>` hint, consistent with malformed `.tp/config.json`. `tp init --eject-roles` writes the corpus and does not read it, so it never aborts on an existing file's contents; `--force` overwrites a malformed target.
...
4.4 A phase directory is **populated** when it contains at least one `*.json` file (non-JSON files are ignored). A populated phase directory **replaces** — does not merge with — the embedded default corpus for that phase: the active corpus is exactly its files (subject only to domain filtering, whose empty-panel fallback is §6.2). When a phase directory is absent or empty, tp uses its embedded default corpus for that phase (§5). A project therefore opts in per phase: custom reviewers with default auditors is valid.
...
8.4 **Cluster representative and attribution.** The cluster's representative supplies its displayed fields and is chosen by a total order: highest `severity`, then lexicographic `role`, then the finding text — deterministic even when two same-role findings tie on severity. (All members of a cluster share the location key by construction, so it is not a tiebreaker.) The cluster carries `found_by_roles` (the sorted set of distinct contributing **diversity** reviewer roles; the built-in `regression` role and absent/blank roles are excluded) and `found_by` (its cardinality). A cluster produced only by `regression` and/or absent-role findings has `found_by = 0` and no `found_by_roles`.

8.5 **Per-role overlap report (no scalar ratio, no auto-taper).** For each diversity reviewer role that appears in the round's findings (the built-in `regression` is excluded — it is never a trim target), `--merge`, `--report`, and `--status` report its `unique` (clusters where it is the sole contributor) and `shared` (clusters it is in with `found_by ≥ 2`). A role with **`unique == 0` and `shared ≥ 1`** — it found only findings others also found — is flagged a **trim candidate**. A role that found nothing is simply absent from the report and flagged nobody, so the certifying clean rounds flag no one. The report is computed from the merged findings alone (each carries its contributing `role`), needing no corpus context. tp only reports; trimming is the orchestrator editing a role file (§4). No corpus-global ratio is computed (no decision consumes one).

8.6 **Mechanize candidates unchanged.** The existing cross-round class-frequency `mechanize_candidates` output is retained exactly as-is. v0.25.0 adds no per-round multi-role mechanize signal and no `found_by` threshold constant: the per-role overlap report (§8.5) already surfaces which reviewers repeatedly overlap, and a `found_by ≥ 2` mechanize rule would only duplicate §8.5's `shared` boundary (Principle 4).

## 9. Role Staleness

9.1 tp derives a corpus hash per phase: **`review_roles_hash`** and **`audit_roles_hash`**. Each is the sha256 over that phase's user role files sorted by repo-relative path, hashing each file's `(repo-relative-path, content with CRLF normalized to LF)`. Repo-relative paths plus line-ending normalization keep the hash identical across clones regardless of checkout path or git `autocrlf`/`eol` settings. When a phase has no user files (embedded corpus), its hash is the fixed sentinel `"builtin"`. The built-in `regression` role is never part of `review_roles_hash` (§5.2); a spec-frontmatter override is covered by `spec_hash`, not the role hash (no double-count).
