...
2. `CLAUDE.md` and `skills/tp/SKILL.md` currently instruct `tp init --quality-gate "…"`. Both recipes
   drop the flag. Without this the next release's `tp init` re-creates the override and the outcome
   above is true only until the next cycle starts. This is a change to what the documents recommend,
   not to the flag inventory of §8.3, which records what exists.
...
| `REFERENCE.md` | Exhaustive field, exit-code and schema detail | Either, on demand |
...
Non-Goal 7 excludes; and observing emissions over this repository's own specs sees only a third of
the rules, so observation cannot source the set either.
...
CLI does not register. The inventory records what exists, not what to use, so §5 removing a flag
from a recommended recipe does not remove it from here.
