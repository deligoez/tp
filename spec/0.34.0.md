...
**This version depends on v0.33.0.** Scope-aware audit convergence is what gives `tp run` a
terminating audit loop; without it the driver would faithfully iterate forever on codebase-scoped
findings, which is exactly what a human had to stop by hand in v0.32.0. Do not start v0.34.0
implementation before v0.33.0 ships.
