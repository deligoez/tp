...
`spec/0.31.0.tasks.json` and `spec/0.34.0.tasks.json`. `quality_gate` is in `readOnlyWorkflowFields`,
so no command removes it; `spec/0.34.0.tasks.json` holds no tasks yet and is re-created by `tp init`
without the flag. `spec/0.31.0.tasks.json` is a shipped record of a released cycle and is left
alone, as Non-Goal 6 requires.

`CLAUDE.md` and `skills/tp/SKILL.md` currently instruct `tp init --quality-gate "…"`; that flag comes
out of both, which is what stops the next release from re-creating the override.

A config-assertion guard test asserts that the gate resolved for `spec/0.34.0.tasks.json` equals the
literal `go test -race ./... && golangci-lint run`. Resolving against the project value instead
would pass on a weakened project gate, since the project value is one side of that comparison.
...
`1508-1543`. Merging that pair requires a new abstraction to hold six differing values, so Non-Goal
7 applies and it is exempted with that reason at the site; each test clone is merged or exempted the
same way.
...
the list to `spec/0.34.0-guard-tests.md`, one `file:function` per line: every test function added or
modified between the `v0.32.0` and `v0.33.0` tags. Measured: **100 test functions across 18 files**.
That window is the evidence-driven one — three of the guards written during it were found not to
guard — and it is reproducible from two tags with no judgement about which commit is an audit round.
The derivation must pass `--no-ext-diff`: this repository configures an external diff driver whose
output is not a unified diff, so the same command without that flag silently yields an empty list.
...
say "about 20". The engine gains an exported enumeration of its lint rule identifiers and emits rule
identifiers **from** it, so the binding is by construction rather than by observation: a rule absent
from the enumeration cannot be emitted. Checking the enumeration against emissions instead would
prove little, since only 7 distinct rules fire across all of this repository's specs. The exact
count is settled by the enumeration rather than by this sentence. This is the durable half — a prose
table that nothing checks will drift again within two releases.
...
Leaving that list makes `TestFileErrorsCarryAHint` enumerate every `ExitFile` site in `lint.go`, and
there are two: the missing-spec path and the `output.JSON` encode failure. Both gain a hint in the
same task, or the gate closes red on the second.
...
Where an item and its section state the same requirement, the item is the normative form: it is what
gets written as an assertion. Rounds 1 and 2 produced five contradictions of this shape, all from
the same section-and-item wording drift.

1. `tp import`, `tp init` and `tp add --spec` each exit 4 with `LockTimeoutError`'s hint when the
   lock is held past `lock_timeout_seconds`. Deterministic, and it fails today at all three entries,
   where none of them takes the lock at all.
2. `tp add --bulk` and `tp set --bulk` abort with exit 3 on a line over the shared cap, name the
   file and line, and leave the task file byte-identical.
3. The `line-cap:` marker guard no longer exempts the bulk readers.
4. The gate resolved for `spec/0.34.0.tasks.json` equals `go test -race ./... && golangci-lint run`.
5. `tp <unknown-command>` and an unknown subcommand exit 2; existing exit codes are unchanged.
6. Each `ExitValidation` site carries a hint naming the file that site operates on, enumerated the
   way `ExitFile` sites are.
7. Both `ExitFile` sites in `lint.go` carry a hint, and `tp lint` emits `specFileMissingHint` on a
   missing spec.
8. The documented lint rule table equals the engine's exported rule enumeration.
9. The documented flag inventory equals the flags the CLI defines.
