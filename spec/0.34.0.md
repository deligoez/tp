...
`workflow.checks` run in the review phase only, while the list is written afterwards, so `PENDING`
is the only state review will ever see the check in. **That task's acceptance is therefore to run
...
Without that acceptance the check never runs against a written list at all.
