...
Every row derives from a numbered decision and names a mutant that must fail it. It does **not** also
claim that every row names an artifact — an earlier form of this sentence did, and every row whose
`from` column is a bare section reference falsified it. That is the
quantifier-over-an-unenumerated-set shape this repository keeps paying for, asserted here over the
release's own seven-row table.
