...
`spec-coverage` therefore keeps a dedicated routing path. It is not the *only* reserved id — `regression` is reserved in both phases by the shared role parser, which this release does not touch (§4 states the same distinction and pins the wording the docs must use). The claim this release makes is narrower and is stated exactly: **no id other than `spec-coverage` changes any part of prompt construction.** §7 item 2 pins it with the fixture §2.9 defines, because the property is not observable by comparing two roles inside one audit run — see §2.9 for why.
...
The bare role ids `security` and `maintainability-conventions` are deliberately absent from that set, on a narrower ground than "a fixture role name carries no behavior" — items 1, 2 and 7 below are listed precisely because a test keyed on one of those names does change. The ground is that the ids alone do not *discriminate*: they appear in 37 committed tests, 27 of which are unaffected, so searching them would bury the ten that matter. The affected ones are reached instead by the symbol and prompt literals above, or listed on their own merits. The two reached only by the bare ids were hand-checked and are green: `TestInitEjectRoles_Output` asserts only the eject JSON keys, which §3.4 leaves untouched, and `TestAuditSchema_PromptFieldsAndOutputSchema` asserts block presence rather than block order, so §2.2's inserted `## Disposition` leaves it green. The list itself is the contract — it carries no count, because a count is one more thing that can drift out of step with it:
...
9. `TestAuditPrompts_AffectedFilesHeaderForms` (`internal/cli/audit_prompts_test.go`) is a **new** test this release adds, not an existing one it invalidates, but it asserts the literal `## Affected Files (max 20)` as `spec-coverage`'s correct untruncated form and therefore matches the search set. It is named here so the inventory accounts for every reference the search returns — the check of §2.10 compares reference against inventory, and an unlisted reference is drift whether the test is old or new.
...
Writing it paid for itself four times before it ever guarded a round, and each payment was a defect in the spec or in the check itself. (1) It proved the original literal set was over-broad — the bare role ids matched 37 tests, 27 unaffected — which is why §2.8 searches symbols and prompt literals instead. (2) Its own first version mis-parsed the spec, ending the section early because the literal set contains lines beginning with `##`; the fix is to track fenced blocks when scanning for a section boundary. (3) The literals `sel.Security` / `sel.Maintainability` were scoped to a variable name rather than a field, so `TestFilterFiles_Cap20`'s `sel2.Security` matched nothing — a test §2.8 itself names as invalidated; the literals are now `.Security` / `.Maintainability`. (4) Its `enclosing_test` attributes an occurrence to the last preceding `func Test`, so an occurrence in a package-level fixture declared before every test — `setupThreeRoleAudit` is exactly that — was silently discarded; such occurrences are now reported by file. All four are recorded because a check that is trusted without being tested is worth less than no check.
...
|---|---|---|---|---|---|
| `security` | +up to 50 | +~5 | +a few chars | −(old − 10), old = min(matches, 20) | +~45 at best |
...
`security` is the only role with any saving at all, because §1 establishes that every other role already ran on the `default` branch's list at `MaintainabilityFileCap`. Even for `security` the saving is conditional, not a flat 10 — and below 10 matches it is not a saving at all. Its old list held `min(matches, 20)` entries where the shared list is a flat `CodeFileCap` of 10, so the table's `−(old − 10)` is a **signed** delta with four cases. A repository matching **fewer than 10** files sees its `security` list *grow* by `(10 − old)` lines, because the shared list backfills to `CodeFileCap` where the old filtered list stopped at the match count; **exactly 10** is neutral; **between 10 and 20** saves `(old − 10)`, so 12 matches saves 2; and **20 or more** saves the full 10. The `+~45` in the table is therefore the best case, reached only at 20 matches or more, and the worst case for `security` is `+~65` — a repository whose keyword list matched nothing pays a custom role's `+~55` plus the 10 backfilled file lines. Only `maintainability-conventions` skips the conventions block's cost, because it alone already received it. What the growth buys is a role that can no longer report against conventions it was never shown (§2.3) and can no longer manufacture a PARTIAL on an out-of-lens file (§2.2), each of which cost a resolve round-trip measured in whole rounds rather than lines. The honest summary: this release trades a conditional and often-zero file-list saving, plus a larger fixed prompt preamble, for a role panel that actually runs — and claims to be free nowhere.
...
10. A role whose pre-cap total equals its list length renders `## Affected Files (max 10)`; a role whose pre-cap total is larger renders `## Affected Files (10 of 34)` with its own list length and total; a `spec-coverage` prompt with nothing truncated renders `## Affected Files (max 20)`. One fixture discriminates §2.5's definition of `CodeFilesTotal`: a universe holding both auditable files and files the drop rules remove (a `testdata/` path, a `.png`) must report the **post-drop** count, so an implementation totalling the raw universe fails.
...
14. A guard test asserts the §3.2 and §3.3 rewrites actually landed, not merely that Go-isms left: `security.json`'s `focus` equals the six §3.2 lines verbatim, `maintainability-conventions.json`'s `focus` equals the five §3.3 lines verbatim, and its `instructions` contains `only whether the code is maintainable and consistent with the conventions already present in this project`. Item 13's blacklist alone passes against an emptied `focus` array.
15. A guard test asserts that `README.md`, `skills/tp/SKILL.md`, `skills/tp/REFERENCE.md`, and `CLAUDE.md` each contain the substrings `spec-coverage is the only auditor id that changes routing` and `ejected role files are not rewritten on upgrade`, and that none contains `file-sec-` or `file-maint-`. It further asserts that `skills/tp/REFERENCE.md` and `skills/tp/SKILL.md` each contain the two §4 prompt-body orders verbatim, that `REFERENCE.md` contains `file-<role-id>-<slug>`, and that both contain `any active role id from the corpus` while containing neither `always \`"implementation-auditor"\`` nor the three-id enum rendering `spec-coverage` \| `security` \| `maintainability-conventions`. Every §4 requirement is then guarded by a required substring rather than only by the absence of an old one.
16. `tp init --eject-roles` emits the §3.4 line verbatim on stderr for `--domain software` and `--domain prose`, emits it with stdout piped (the JSON-mode case `output.Info` would swallow), does not emit it under `--quiet`, and leaves the stdout JSON keys exactly `{ejected, domain}`; the existing byte-identity test still passes.
17. `priorityPathSubstrings` is asserted to equal exactly `["lock", "validate", "auth", "secret", "perm"]`, mirroring the cap-literal guard of item 7: the rename from a filter to a ranking signal is the moment a value is most likely to be quietly dropped or added, and items 4 and 5 exercise only one keyword each.
18. `SpecCoverageTotal` is pinned on both of its branches: on a spec whose task file maps fewer files than the diff touches and whose task-mapped set exceeds `AuditFileCap`, the `spec-coverage` header renders `## Affected Files (20 of <task-mapped size>)`; with no task file, the total equals the filtered-universe size. Without both cases an implementation that always uses the filtered universe passes.
