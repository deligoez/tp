...
The inventory below is exhaustive against a search for every committed reference to `sel.Security`, `selectSecurity`, `HeadReader`, `securityHeadLineCap`, `headLines`, `containsSecuritySubstring`, `sel.Maintainability`, `selectMaintainability`, `MaintainabilityFileCap`, `roleSecurity`, `roleMaintainability`, `## Project Context`, the literal `## Affected Files (max 20)`, and the JSON role keys `security` / `maintainability-conventions`. The list itself is the contract — it carries no count, because a count is one more thing that can drift out of step with it:
...
7. `TestAudit_FileFilterCap` (`internal/cli/integration_test.go`) supplies 50 path-matching files and asserts the `security` prompt's bound. Under §2.4 that role's list is bounded by `CodeFileCap`, so the expected count becomes 10 and the assertion is retargeted rather than deleted — it is the only committed test exercising the cap end-to-end.
...
In a repository where the keyword list already matched, the accounting is three deltas up against one down, per shared-arm prompt: §2.3 adds up to 50 lines of project conventions, §2.2 adds the ~5-line `## Disposition` block, §2.6 adds a few characters to one header — against up to 10 fewer affected-file path lines, since the cap falls from 20 to 10. That is a growth of roughly 45 lines for a paths-only role, not a wash; the earlier draft's "roughly unchanged" did not survive its own arithmetic. What the growth buys is a role that can no longer report against conventions it was never shown (§2.3) and can no longer manufacture a PARTIAL on an out-of-lens file (§2.2), each of which cost a resolve round-trip measured in whole rounds rather than lines. The honest summary: this release trades a narrower file list and a larger fixed prompt preamble for a wider role panel, and claims to be free nowhere.
...
14. A guard test asserts that `README.md`, `skills/tp/SKILL.md`, `skills/tp/REFERENCE.md`, and `CLAUDE.md` each contain the substrings `spec-coverage is the only reserved auditor id` and `ejected role files are not rewritten on upgrade`, and that none contains `file-sec-` or `file-maint-`.
