...
3.2 **Round-trip preservation.** Every command that acquires the task-file write lock and rewrites the file — currently `tp done`, `tp close`, `tp set`, `tp reopen`, `tp remove`, `tp add`, `tp claim`, `tp commit`, and `tp import` — leaves the set of present `workflow` fields unchanged, **except for the single field a `tp set --workflow <field>=<value>` intentionally adds or updates** (§6.1). A `"workflow": {}` block stays `{}`; a block with only `review_max_rounds: 4` keeps exactly that one field; a source file that arrives with no `workflow` key at all is normalized to `"workflow": {}` on the next write (§4.4), never populated with defaults.
...
6.1 `tp set --workflow <field>=<value>` adds or updates exactly that field in the task file's `workflow` block, leaving every other field's presence untouched (it never materializes siblings). The value is validated **before** the file is written: an out-of-range value (e.g. `review_clean_rounds=0`, outside 1–10, or `gate_timeout_seconds` outside 30–3600) is rejected with exit code 1 and an actionable hint, and the task file is left unmodified. Unsetting a field is out of scope for this release (§9).
...
10.2 **Round-trip:** a task file with `"workflow": {}` survives `tp add`, `tp claim`, `tp done`, `tp close`, `tp remove`, `tp commit`, `tp import`, `tp set` (a non-workflow field), and `tp reopen` with its `workflow` block still `{}`. A task file with only `{"review_max_rounds": 4}` keeps exactly that one field across the same operations. A source file with no `workflow` key is normalized to `{}` on the next write, not populated with defaults.

10.3 **No masking:** with `.tp/config.json` `{"workflow": {"review_clean_rounds": 3}}` and a `tp init`-created task file, `tp config --resolved` reports `review_clean_rounds: {value: 3, source: "project"}` (not `2`/`task`), and `tp review <spec> --status --check` exits non-zero until 3 clean rounds are recorded, then exits 0.

10.4 **Explicit override still wins:** a task file that sets `review_clean_rounds: 5` resolves to 5 over a project config's 3. An invalid explicit set `tp set --workflow review_clean_rounds=0` exits 1 with a hint and leaves the file unmodified.
...
10.7 **`commit_strategy` round-trip:** a task file with `{"commit_strategy": "squash"}` keeps exactly that field across the §10.2 operations, and `tp config --resolved` attributes it to `source: "task"`.
