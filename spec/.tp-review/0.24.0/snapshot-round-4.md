...
4. A type-mismatched value — `active` that is not a string, a `defaults` entry that is not a
   boolean, or a `workflow`/`defaults` top-level value that is not an object — is reported as a
   validation warning on stderr and treated as unset (ignored), not an error.
...
4. `tp config --extract --dry-run` prints the hoist/removal plan without writing. Without
   ...
   hand-set project fields. All writes acquire the standard flock.
