...
2. The upward walk halts at the repository boundary — the first ancestor containing a `.git`
   ...
   filesystem root, whichever comes first, and never reads a `.tp/` above that boundary; this
   ancestor is the **git boundary**.
...
4. When no `.tp/` directory is found, tp uses built-in defaults and per-task-file settings
   exactly as in v0.23.0.
5. The **project root** is the directory containing the discovered `.tp/`. When no `.tp/` exists
   yet, commands that need a project root — config creation (§4) and the project-wide scan
   (§10) — use the git boundary directory instead, or the working directory when not inside a
   git repository.
...
4. A type-mismatched value — `active` that is not a string, a `defaults` entry that is not a
   boolean, a `workflow`/`defaults` top-level value that is not an object, or a `workflow` field
   of the wrong JSON type (a non-number where a number is expected, or a non-array `checks`) — is
   reported as a validation warning on stderr and treated as unset: the field is ignored and
   falls back to its inherited or built-in value, not an error.
...
1. `tp set --workflow --project field=value` writes the field to the `workflow` block of the
   ...
   (and `.tp/` plus `.tp/.gitignore`) in the git boundary directory that contains `.git`
   (whether `.git` is a directory or a file) — or, when not inside a git repository, in the
   current working directory. That directory then becomes the project root (§2.2.5).
...
2. `tp config --resolved` annotates each resolved setting with a `value` and the `source` layer
   that supplied it. The `source` enum is `cli`, `env`, `override`, `project`, `local`,
   `legacy`, `autodetect`, or `default`: workflow fields report `override`/`project`/`default`;
   `active` reports `cli` (`--file`), `env` (`TP_FILE`), `local` (`.tp/local.json`), `legacy`
   (`.tp-active`), or `autodetect`; `defaults` values from `.tp/local.json` report `local` (not
   `project`, which is reserved for the committed `.tp/config.json`). Example:
...
1. `.tp/local.json`'s `active` key holds a path relative to the project root (the directory
   ...
   auto-detect.
2. `tp use <file>` resolves its argument (CWD-relative or absolute) to an absolute path and
   stores it project-root-relative in `.tp/local.json:active` (creating `.tp/`, `.tp/.gitignore`,
   and `.tp/local.json` as in §4.1); a target outside the project root is rejected with exit 2. It
   no longer creates a `.tp-active` file.
...
4. Passing a flag and its negation together in one invocation (e.g. `--compact --no-compact`) is
...
3. Equality for the `checks` array is order-insensitive set comparison over `{class, cmd}`
   pairs; a reordered but equal `checks` is not a deviation. A `checks` deviation is reported as
   `<file>: checks differ from project (N vs M entries)`, not as a scalar value.
...
1. `tp validate --project` and `tp config --extract` share one discovery rule: starting at the
   project root (§2.2.5 — the directory containing the discovered `.tp/`, or the git boundary /
   working directory when no `.tp/` exists yet, the `tp config --extract` bootstrap case), it
   collects every `*.tasks.json` in that directory and, recursing downward, its subdirectories.
...
4. A `*.tasks.json` that is malformed or unreadable during the scan is reported on stderr and
   skipped by `tp validate --project` (the scan continues); `tp config --extract` instead aborts
   with exit 3, because a correct hoist requires reading every task file.
...
3. `tp config --extract` hoists a workflow field — including the string `quality_gate` and the
   `checks` array (compared by the §9.3 set equality) — into `.tp/config.json` only when
   **every** discovered task file sets that field with an identical value; a field absent from
   any file, or whose value diverges across files, is left untouched in every file. It then
   removes each hoisted field from every task file's `workflow` block. When no field is common
   to all files, `--extract` writes nothing, reports that there was nothing to hoist, and exits 0.
4. `tp config --extract --dry-run` prints the hoist/removal plan without writing. Without
   ...
   a non-empty `git status --porcelain` — with exit 4 so the mutation is reviewable; outside a
   git repository it skips the clean-tree check and warns. It refuses when `.tp/config.json`
   already exists (exit 4) unless `--force`, which **merges** the hoisted fields into that file's
   existing `workflow` block — overwriting only the hoisted keys and preserving any other
   hand-set project fields. `--force` governs only the existing-config refusal; it never bypasses
   the dirty-working-tree gate, which always holds because `--extract` mutates task files. All
   writes acquire the standard flock.
