...
3. When tp first creates the `.tp/` directory it also writes `.tp/.gitignore` containing
   `local.json`, so `.tp/local.json` is ignored automatically without user action.
...
2. The upward walk halts at the repository boundary — the first ancestor containing a `.git`
   ...
   filesystem root, whichever comes first, and never reads a `.tp/` above that boundary.
...
2. `.tp/local.json` accepts two top-level keys, `active` (string) and `defaults` (object).
3. Unknown top-level keys, unknown keys inside `workflow`, and unknown keys inside `defaults`
   are ignored and reported as a validation warning on stderr; they are never errors.
4. A workflow value outside its documented range in a hand-edited `.tp/config.json` falls back
...
5. A malformed or unreadable `.tp/config.json` or `.tp/local.json` aborts the command that reads
   it with exit 3 and a `repair or delete <path>` hint; tp never silently ignores a corrupt
   config. Every command that resolves workflow, active, or flag-default values reads these
   files, so any such command can abort with exit 3.
6. A 0-byte or whitespace-only file is malformed JSON and is handled by 2.3.5, distinct from an
   explicit empty object `{}`, which is valid and equivalent to an absent file.
...
| 2 | Environment variable | e.g. `TP_FILE`, `NO_COLOR`; a rank-1 CLI flag overrides its rank-2 env peer |
...
2. `defaults` (flag defaults) has no task-file layer; it is resolved across ranks 1, 2, 4, 5
   only. Workflow fields have no rank-2 env layer. The ladder lists every source; not every
   field passes through every rank.
3. `active` is resolved by the discovery chain in §5.1, not by this workflow ladder.
...
1. `tp set --workflow --project field=value` writes the field to the `workflow` block of the
   `.tp/config.json` discovered by §2.2. When no `.tp/` exists, it creates `.tp/config.json` (and
   `.tp/` plus `.tp/.gitignore`) at the repository root — the directory containing `.git` — or,
   when not inside a git repository, at the current working directory.
...
4. `.tp/local.json` `defaults` are written by `tp set --local defaults.<flag>=<bool>`; `active`
   is written only by `tp use` (§5.1). `--project`/`--local` writes acquire the same flock used
   by every other tp write.
...
2. `tp config --resolved` annotates each workflow field with a `value` and the `source` layer
   that supplied it, one of `cli`, `env`, `override`, `project`, `default`. Example:
   ...
   ```json
   {
     ...
     "active": { "value": "chapter-03/chapter-03.tasks.json", "source": "project" },
     "defaults": { "compact": { "value": true, "source": "project" } }
   }
   ```
...
1. `.tp/local.json`'s `active` key holds a path relative to the directory containing the
   discovered `.tp/`; it takes the place of the `.tp-active` marker in the discovery chain:
   `--file` > `TP_FILE` > `.tp/local.json:active` > legacy `.tp-active` > auto-detect.
...
2. An `active` value that is absolute or escapes the repository boundary is rejected with a
   validation warning and treated as unset.
...
1. `tp validate --project` scans every task file under the project (§10) and reports, per file,
   each workflow field whose override differs from the project config value, naming the file,
   the field, the override value, and the project default:
   `chapter-03/chapter-03.tasks.json: review_max_rounds=0 overrides project default 8`.
2. Equality for the `checks` array is order-insensitive set comparison over `{class, cmd}`
   pairs; a reordered but equal `checks` is not a deviation.
3. Deviations are informational and exit 0 by default; `--strict` promotes any deviation to
...
1. `tp validate --project` and `tp config --extract` share one discovery rule: from the
   directory containing the discovered `.tp/` (the project root), recurse into all
   subdirectories down to the repository boundary, collecting every `*.tasks.json`.
2. The recursion skips `.git`, `.tp`, `node_modules`, and `vendor` directories.
...
3. `tp config --extract` hoists a workflow field into `.tp/config.json` only when **all**
   discovered task files agree on its value; any field whose value diverges across files is left
   untouched in every file as a deviation. It then removes each hoisted field from every task
   file's `workflow` block.
4. `tp config --extract --dry-run` prints the hoist/removal plan without writing. Without
   `--dry-run`, `--extract` refuses to run on a dirty git working tree (exit 4) so the mutation
   is reviewable, and refuses when `.tp/config.json` already exists unless `--force`. All writes
