...
**One deliverable, and it is behaviour-preserving by default.** §2 ships `audit_converge_on` — its
resolution, its fence, its validation and its documentation — and changes nothing for a project that
does not set it. A second deliverable, a spec-hash reset for `consecutive_clean`, was in this release
through five review rounds and is now `spec/0.46.0.md` §8.4; §3 records why. **This paragraph
described the two-deliverable release for one round after the cut**, which is the shape of defect
§3 exists to stop the next reader re-proposing.
...
top two layers cannot be verified because nothing implements them. The five-layer form was copied into
this draft from `CLAUDE.md`; both were corrected during this cycle's review, along with
`REFERENCE.md`'s statement of it, and **that sweep is done rather than owed** — it is recorded here
as provenance, not as scope, because a scope sentence already satisfied cannot fail as acceptance.
`spec/0.31.0.md:41` and `spec/0.35.0.md:568-573` keep their copies: those are shipped specs and
`spec/0.34.0.md` Non-Goal 6 forbids editing one. **And the corrected rule is not absolute**:
`engine/runnerseam.go`'s `ResolveUnitRunner` reads `TP_RUNNER_SEAM` ahead of the `runner` field, and
`notify_cmd` resolves from `.tp/local.json` only. `spec/0.35.0.md` §7 states the rule *with* both
exceptions and is the most accurate text in the repository on this point. `engine/reviewconvergeon.go` is twenty-four lines
...
fields. A string-valued field does not fit it. **The mechanism already exists and is not the numeric one.** `fenceWorkflowWrite`
(`unattended.go:56`) returns early unless `engine.FencedCapField(field)` — a seven-entry numeric map
— and its four call sites (`set.go:414`, `set.go:428`, `set_project.go:98`, `set_project.go:108`) all
sit after `ParseFloat`/`Atoi`, so a string field reaches none of them and "widening the numeric
fence" would ship a silent no-op. `engine/unattended_test.go:69` already asserts a convergence field
is *not* a fenced cap field.

What to copy instead is the **string-shaped fence that ships today**, placed *before* the type
dispatch at `set.go:361-362` and `set_project.go:46-47`:
...
So the work is a third predicate beside `FencedCapField` and `FencedCommandField`, and that guard
copied at both sites. **Both write paths are fenced**: `tp set --workflow audit_converge_on=blocking` and
...
**The fence is on the field, not on the direction.** Both `=blocking` and `=all` are refused under
`TP_UNATTENDED=1`. The cap fence refuses only a *raise* (`unattended.go:69`) because a lower cap is
strictly safer; there is no such asymmetry here, since a unit that can set `all` can set `blocking`
on the next line and a direction-scoped fence is a fence a unit can walk around.
...
recoverable only from free text. This release adds `audit-converge-on` to that enum and maps the field to it — **and the enum's
cardinality is asserted in nine places that must move with it**, counted:
`engine/escalation.go:14,29,39,47` (including a comment reading "not a sixth category"),
`engine/escalation_test.go:60`, `cli/escalate_test.go:158` (an explicit five-literal loop),
`internal/fakerunner/.../main.go:260`, and the two agent-facing tables at `REFERENCE.md:347` and
`SKILL.md:247`. `spec/0.35.0.md:428-429` closes the set over five and is a shipped spec, so it is
recorded as superseded rather than edited. The field is opt-in,
...
are **zero** out-of-enum severities in 238 non-`PASS` rows. §5 test 7 uses a legacy row because it is
...
4. **Review's convergence.** `review_converge_on` is unchanged, and nothing in this release touches
   `engine.ConsecutiveClean` or `engine.Converged` — the shared walks review's `--status` reads. The
   release that will touch them is `spec/0.46.0.md` §8.4, which is why the reset left this one.
5. **A fourth row status.** `AuditRowIsPass` stays byte-exact on `status == "PASS"`.

6. **An audit-side `nonblocking_open` on `--record` and `--status`.** An earlier draft justified this
   by saying tp emits no count of the suppressed rows. **That premise is false and its own §2 says
   so**: `role_streaks[].open` stays severity-blind, is set unconditionally at
   `audit_record.go:152`, and `REFERENCE.md` documents it on both surfaces — so the *count* is
   already there and the missing thing is the *breakdown*, which is much smaller. **The breakdown is
   in scope and ships on `--merge`**, where it is free: `audit_merge.go:61-86` already loops the
   round's rows building `by_status` and `by_role`, and `:90-92` already carries a
   `--compact`-stripped explanatory key, so a `by_severity` counter over non-`PASS` rows is a few
   lines inside an existing loop on a command the loop already runs immediately before `--record`.
   None of the four pins below reaches `--merge`; all four are about `--record` and `--status`, which
   is what stays deferred. The review side emits `nonblocking_open`
   (`review_record.go:231`) Four places pin its absence on `--record`/`--status` by
   name, counted: `spec/0.31.0.md` §8.4 and its test 18, `skills/tp/REFERENCE.md:641`, and
   `reviewconvergeon_clean_test.go:185-205` (`TestReviewNonBlockingOpen_AuditUnaffected`, which
   asserts only that those two commands never emit the key). (`harness_note_test.go:263` is a comment carrying no such
...
**Two shipped guards are deliberately reversed.** The first, and the reversal is exactly two of its
eleven assertions: `internal/cli/audit_signal_test.go`'s `TestAuditGate_NoEscapeHatchFromTheGate` carries
...
`spec/0.46.0.md` §8.4's reset will touch when it ships. Non-Goal 2 keeps the first two true; §5 test 11 pins all nine. One dependency is stated because it is
otherwise invisible, and it is narrower than an earlier draft claimed: only `:490` needs §2's
fail-closed rule to hold, because `signalRow` (`audit_signal_test.go:40-42`) emits no `severity` key.
`:493-:500` read a payload recorded before the reversed line and are unaffected.

**The second guard is `internal/cli/docs_contract_test.go:96`**, which pins the literal
`"audit convergence still counts every non-PASS row"` in `SKILL.md`. `blocking` makes that sentence
untrue, so updating the docs reddens a shipped test and not updating them ships contradictory
documentation. This release updates both, and §5 test 3's doc assertions are over that sentence and
`SKILL.md:145`'s `--status --check` clause, which is also false under `blocking` — named as claims
rather than as filenames, since an earlier draft scoped the docs by file and the two sentences this
release actually falsifies went unnamed for two rounds.
