...
A non-boolean `enabled` is a lint warning and is ignored, leaving the role active. Its text follows the parser's existing shape: `tp.<field>.<id>.enabled is not a boolean (got %T); ignored`. The existing warnings for a non-mapping value, a non-list `focus`, and a non-string focus element are unchanged.
...
3. `internal/engine/lens_shim_test.go` — builds `ReviewRoles` map literals, whose element type changes. (It also calls `TranslateLegacyLens`, but §2.2 leaves that signature to the implementer, so the literals are what invalidate it under either branch.)
