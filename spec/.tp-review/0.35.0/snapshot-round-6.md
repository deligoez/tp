...
passes. `audit-fix`'s `id` is the finding's `(role, item_id)` from the round's merged results.
...
`run_max_wall_clock_seconds` could run itself indefinitely.
