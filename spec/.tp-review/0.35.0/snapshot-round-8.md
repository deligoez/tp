...
most one file per role however many runs contributed to it. The driver deletes that file before each
attempt, so a failed attempt's leftovers can never satisfy the next attempt's predicate.
...
a CLI flag, so a test can pin the runner whatever the repo's config says. The fake runner writes a
...
| `review-role` | One role's findings file for the round | `$TP_ROUND_DIR/$TP_UNIT_ID.ndjson` exists | Parallel with sibling roles |
...
| `audit-role` | One role's results file for the round | `$TP_ROUND_DIR/$TP_UNIT_ID.ndjson` exists | Parallel with sibling roles |
| `audit-record` | The round recorded, convergence recomputed | An audit round file for `$TP_ROUND` exists | Alone |
| `audit-fix` | One audit finding closed, with or without a code change | That finding's row is recorded resolved | Alone |
...
round-based phase `next_units` omits any role whose findings file already exists in the round
directory, so a resumed round runs only the roles it still needs.
...
| `Stop` | Inside a role unit with neither its findings file nor an escalation record, block once with the reason |
...
ends in `-role` and stats `$TP_ROUND_DIR/$TP_UNIT_ID.ndjson` and
...
than per-project and is read from `.tp/local.json` only, and `TP_RUNNER` (§3.2.1) is a test seam
...
**A dropped role does not merge clean.** `tp review --merge` and `tp audit --merge` warn on stderr
...
`{path, parsed, skipped}`, and an input with at least one line, none of which parsed, makes
`--merge` exit 1. A zero-byte file stays the documented way a role reports nothing found and keeps
...
44. The `claude` template appends the documented `--agent` per unit kind, and none for the kinds
    that take no agent.
45. A resumed round's `next_units` omits every role whose findings file is already present.
