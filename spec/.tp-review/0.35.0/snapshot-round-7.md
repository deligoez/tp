...
path. A role's findings file is named by role rather than by `seq`, so a retry overwrites its own
failed attempt and a round directory holds at most one file per role however many runs contributed
to it.
...
`run_max_wall_clock_seconds` could run itself indefinitely. Under the variable the env layer of §7's
precedence is ignored for every fenced field, so exporting a higher cap is not a supported route
either.
...
is set, the durable write is the state that kind names in §3.3's fourth column, and an escalation is
`$TP_RUN_DIR/$TP_UNIT_SEQ-escalation.json`. It never reads the run state. The hook
...
failed" — the distinction a driver branches on. Which kind of 2 it is, a refusal (§5.1) or a typo,
is carried by the escalation record's presence (§5.2), not by a further exit code. `tp show`'s arity
hint names the missing argument rather than claiming to name the failing object.
...
39. The Stop hook blocks at most once per unit: a second invocation with `stop_hook_active` set
    allows the stop.
40. Under `TP_UNATTENDED=1` an environment override of a fenced field is ignored.
