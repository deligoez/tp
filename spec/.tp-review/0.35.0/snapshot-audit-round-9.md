...
without parsing the payload, and `stop_reason` names which of the nine it was.
...
nothing else. The existing `next_action` object stays and stays the human-facing summary; when the
array is non-empty its `payload.unit` names `next_units[0]`'s kind and id, so the two surfaces can be
checked against each other, while `command`, `brief_command` and `summary` keep the phase-level forms
that already address that same unit.
...
means *disabled* rather than *lowest* for the four fields where 0 is unbounded — `review_max_rounds`,
`audit_max_rounds`, `run_max_budget_usd` and `run_max_unit_budget_usd`: setting any of them to 0
while the resolved value is non-zero is a raise and is refused, while setting 0 where 0 already
resolves changes nothing and is accepted. `run_max_unit_retries` is the contrast — its 0 is simply
the fewest attempts, so it is always accepted. The driver's own caps are fenced on
...
37. Every child receives the documented environment variables in their documented forms —
    ...
    phase.
38. `next_units` carries `{kind, id, brief_command}` plus `round`, `next_action.payload.unit` names
    `next_units[0]`'s kind and id — while `command`, `brief_command` and `summary` keep the
    phase-level forms this section gives the human-facing summary, so the two halves can be checked
    against each other — and a non-concurrent kind is never returned alongside another unit.
