...
Every child is spawned with `TP_RUN_ID`, `TP_UNIT_ID`, `TP_UNIT_KIND`, `TP_UNIT_SEQ` and
`TP_RUN_DIR` in its environment, applied on the same terms as `TP_UNATTENDED` (§3.2). They are what
let a unit address its own artifacts without the driver passing them through prose.
...
`["{unit_kind}", "{unit_id}", "{log_path}"]`, and it outranks every other source of `runner`. It is
the env layer of the existing precedence, so no new precedence rule is introduced.
...
| `review-role` | One role's findings file for the round | `$TP_RUN_DIR/$TP_UNIT_SEQ-$TP_UNIT_ID.ndjson` exists | Parallel with sibling roles |
| `review-record` | The round merged and recorded | A round file for round N exists | Alone |
| `review-resolve` | The spec revised and each addressed finding resolved | The spec's hash changed | Alone |
| `decompose` | Tasks imported into the task file | The task file holds at least one task | Alone |
| `audit-role` | One role's results file for the round | `$TP_RUN_DIR/$TP_UNIT_SEQ-$TP_UNIT_ID.ndjson` exists | Parallel with sibling roles |
| `audit-record` | The round recorded, convergence recomputed | An audit round file for round N exists | Alone |
| `audit-fix` | The code change for one audit finding committed | `HEAD` advanced | Alone |

The durable-write column is the Stop hook's predicate (§6.2) and the driver's own success test, so
both read the same condition.
...
is set, the durable write is the condition that kind names in §3.3's fourth column, and an
escalation is `$TP_RUN_DIR/$TP_UNIT_SEQ-escalation.json`. It never reads `.tp/run.json`. The hook
...
`$TP_RUN_DIR/$TP_UNIT_SEQ-$TP_UNIT_ID.ndjson` — and are denied every other write. When `TP_RUN_DIR`
is set, `tp review` and `tp audit` emit that same path as the role's output file, so the prompt and
the allowlist name one filename rather than two; the `review-record` and `audit-record` units merge
`$TP_RUN_DIR/*.ndjson` for the round. Outside a run the emitted filename is unchanged.
...
16. Under `TP_UNATTENDED=1`, each of the three user-only operations exits 2 with its hint; a cap set
    to an equal or lower value exits 0; and each operation exits 0 without the variable.
17. A `runner.env` entry for `TP_UNATTENDED` does not reach the child.
18. An escalating unit stops the run with artifacts intact, is not counted as a unit failure, sets no
    `last_failure`, and invokes `notify_cmd` when configured. An exit 2 with no escalation record is
    a unit failure instead.
19. `tp escalate` writes the per-unit record and exits 2; two concurrent siblings each write their
    own; outside a run it is a usage error.
20. `last_failure` is written to `.tp/last_failure.json` on a non-zero unit exit, surfaced by `tp
...
21. The plugin validates with `claude plugin validate`, and its hook payloads are asserted against
    fixtures.
22. The `PreToolUse` deny covers `.tp-review/`, `*.tasks.json`, `.tp/config.json` and
    `.tp/local.json`, and does not fire for tp's own writes through a shell.
23. The Stop hook does not block a unit that wrote an escalation record, and does block one that
    ended with neither its durable write nor a record.
24. Under `TP_RUN_DIR`, `tp review` and `tp audit` emit the run-scoped findings path, and the record
    unit merges the round's files from it.

### 10.5 Signals the driver reads

25. `location_clusters` groups a location reported by two roles under different classes, and leaves
    the recorded finding count, the `clean` flag and the `--status --check` exit code unchanged.
26. `next_action`'s mechanize branch carries the phase qualifier.
27. A file set truncated by the cap reports `truncated` true and `total_changed` above
    `total_files`, under `--quiet` as well.
28. An arity violation exits 2 for every command, including cobra's own built-ins, and `tp show`
