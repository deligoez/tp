...
`TP_RUN_DIR`, `TP_ROUND_DIR` and `TP_FILE` in its environment, applied on the same terms as
...
`.tp/rounds/<base>/<phase>-r$TP_ROUND`, keyed by the cycle rather than the run, so a round
...
omitting roles whose file is already present (§4.1); the driver then deletes the file of every unit
that set does contain, immediately before spawning it. A leftover therefore either means the role is
...
| `review-role` | One role's findings file for the round | `$TP_ROUND_DIR/$TP_UNIT_ID.ndjson` exists and every line parses | Parallel with sibling roles |
| `review-record` | The round merged and recorded | A review round file for `$TP_ROUND` exists | Alone |
| `review-resolve` | The spec revised and each addressed finding resolved | Every finding in the round's file carries a disposition | Alone |
| `decompose` | Tasks imported into the task file | The task file holds at least one task | Alone |
| `audit-role` | One role's results file for the round | `$TP_ROUND_DIR/$TP_UNIT_ID.ndjson` exists and every line parses | Parallel with sibling roles |
...
duplicate> "<evidence>"`, the audit-side mirror of `tp review --resolve` — same 0-based index, same
dispositions, same `--resolve-all` and `--force`. Without it the kind has no way to record the
no-code-change outcome §3.3 admits, and the audit side already mirrors `--merge`.

Every predicate is a state, never a delta: each is decidable by reading one file, with no baseline
and no "before". That is what lets the Stop hook (§6.2) and the driver's own success test read the
same condition, and it is why a unit that correctly closes a finding without changing code still
passes.

Each kind's brief comes from the `brief_command` the oracle supplies for it (§4.1). A unit succeeded
...
`escalation`, `unit-failure`, `interrupted`, `cap-budget`, `cap-wall-clock`, `cap-units`,
`no-units` — `converged` leads because §3.1 checks releasability before caps, and a cycle that
...
Each unit's runner output streams to `$TP_RUN_DIR/<seq>-<kind>-<id>.jsonl`. The driver reads
...
each carrying `{kind, id, brief_command}`, alongside `round`: the phase's current round number, or
null outside a round-based phase. `round` and the existing `phase` are what the driver substitutes
...
at `.tp/last_failure-<base>.json`: `{unit_kind, unit_id, phase, exit_code, summary, at}`. `summary` is
...
exits 0, since lowering a budget cannot manufacture convergence. The driver's own caps are fenced on
...
Under the variable the env layer of §7's precedence is ignored for every fenced field. The layer is
`TP_` + the upper-cased field name (`TP_RUN_MAX_UNITS`, `TP_REVIEW_MAX_ROUNDS`, and so on), which is
what a test sets to prove it is ignored.
...
is not counted as a failed attempt, and does not set `last_failure`. A unit that wrote none, or one
...
ends in `-role` and applies §3.3's predicate to `$TP_ROUND_DIR/$TP_UNIT_ID.ndjson` — present and
wholly parseable, the same condition the driver and the oracle use — alongside
`$TP_RUN_DIR/$TP_UNIT_SEQ-escalation.json`. Every other kind's predicate needs tp's own state
...
`$TP_ROUND_DIR/$TP_UNIT_ID.ndjson` — and are denied every other write. When
...
task override > project config > built-in — with two exceptions: `notify_cmd` is per-operator rather
than per-project and is read from `.tp/local.json` only, and `TP_RUNNER_SEAM` (§3.2.1) is a test seam
that outranks every layer including CLI.
...
18. Each durable-write predicate is decidable from one file with no baseline, and an `audit-fix`
    that closes its finding without a code change passes.
...
32. `--merge` reports `inputs` with per-file parsed and skipped counts, exits 1 when an input has
    lines and none parsed, and exits 0 when every input is zero-byte — for both merges.
...
37. Every child receives the eight documented environment variables with their documented forms:
    `TP_RUN_ID` a ULID, `TP_RUN_DIR` absolute, `TP_ROUND_DIR` derived from phase and round, and
    `TP_FILE` the driver's resolved task file.
...
39. The Stop hook blocks at most once per unit: a second invocation with `stop_hook_active` set
    allows the stop.
40. Under `TP_UNATTENDED=1` an environment override of a fenced field is ignored, and `runner` and
    `notify_cmd` cannot be set at any layer.
...
44. The `claude` template appends the documented `--agent` per unit kind, and none for the kinds
    that take no agent.
45. A resumed round's `next_units` omits every role whose findings file is already present, and the
    driver deletes the file of every unit the oracle did return before spawning it.
...
53. `tp audit --resolve` and `--resolve-all` dispose audit rows on the same terms as their review
    counterparts, and an `audit-fix` unit that disposes its row without a code change succeeds.

### 10.7 Existing tests this change invalidates
