...
file of every unit that set does contain, immediately before spawning it. A leftover therefore either means the role is
...
dispositions, same `--resolve-all` and `--force`. The results file is the one the round's
...
loop starts. A driver's caller therefore separates "the cycle is done" from "a human is needed"
...
means *disabled* for both budget fields rather than *lowest*: setting either to 0 is a raise and is
refused. The driver's own caps are fenced on
...
wholly parseable — alongside `$TP_RUN_DIR/$TP_UNIT_SEQ-escalation.json`. The predicate is two
...
45. A resumed round's `next_units` omits every role whose findings file satisfies §3.3's predicate,
    and the driver deletes the file of every unit the oracle did return before spawning it.
