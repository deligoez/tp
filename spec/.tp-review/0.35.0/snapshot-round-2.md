...
`review-resolve` and `decompose`. Ids are unique within a run and are what `last_failure` clears on.

`tp run` holds the same exclusive lock tp's writes take, so a second `tp run` over the same task
file exits 4 rather than spawning duplicate units. A crashed `implement` unit leaves its task `wip`;
the next run's `implement` unit resumes that same task, which is what `tp next` already does.
...
| `args` | Argument template; placeholders `{prompt}`, `{max_budget_usd}`, `{unit_id}`, `{log_path}` |
...
Two built-in templates ship by name: `claude` and `opencode`. The `claude` template's `args` are
`["-p", "{prompt}", "--output-format", "stream-json", "--verbose", "--permission-mode", "auto"]`,
with `["--max-budget-usd", "{max_budget_usd}"]` appended when the resolved `run_max_unit_budget_usd`
is non-zero. Every child is spawned with stdin closed; a backgrounded run that inherits a TTY-less
...
`{unit_id}` expands to the unit's id and `{log_path}` to its log path (§3.5). A placeholder the
driver cannot resolve is a usage error (exit 2) raised before any child is spawned.
...
A map missing `default` is a usage error. A `run_max_unit_budget_usd` of 0 omits the budget flag
entirely rather than passing a literal 0.
...
The built-in `opencode` template's `args` are `["run", "{prompt}"]`, with no budget flag and no
`spend_key`, so `cap-budget` is inert for it.
...
| `review-resolve` | The spec revised and each addressed finding resolved | Alone |
| `decompose` | Tasks imported into the task file | Alone |
...
| `audit-fix` | The code change for one audit finding committed | Alone |
...
progress file that no decision reads back is the way to avoid it here. The accrued totals therefore
restart at zero on a new run: caps bound a run, not a cycle.
...
only when `unit_id` matches. It lives outside `.tp/run.json` because it must survive the run that
wrote it; it is git-ignored, and it is advisory — its absence never changes which unit runs next.
...
writes `.tp/runs/<run-id>/escalation.json` — `{decision, unit_kind, unit_id, phase, evidence,
options[], at}` — and exits 2. The unit is the writer; a tp command that refuses under §5.1 prints
the hint and the unit records the escalation from it.

**The record, not the exit code, is the signal.** A unit that exits 2 having written the record
stops the run with `stop_reason: escalation`, is not counted as a failed attempt, and does not set
`last_failure`. A unit that exits 2 without one is an ordinary unit failure. At most one record is
written per run; a second escalation in the same run cannot occur, because the first stops it.
...
| `Stop` | If the run marker is present and the unit's durable write is absent, block once with the reason |
...
The stop hook's two predicates are files, not inferences: the run marker is the unit's entry in
`.tp/run.json`, and the durable write is the artifact its kind names in §3.3. The hook must consult
`stop_hook_active` and block at most once per unit. A hook that always blocks produces an agent that
can never finish, which is the loop this whole version exists to make terminating.
...
`.tp/runs/<run-id>/<seq>-<role>.ndjson`, which the `review-record` and `audit-record` units then read
and merge — and are denied every other write. The definitions carry tool restrictions only: review
and audit role content stays in the corpus (`.tp/reviewers/`, `.tp/auditors/`) and reaches the unit
through the prompt `tp review`/`tp audit` already emits, so no role text is duplicated here.
...
7. `last_failure` is written to `.tp/last_failure.json` on a non-zero unit exit, surfaced by `tp
   resume` and `tp brief`, survives into the next run, and is cleared on the next success of that
   unit.
8. Under `TP_UNATTENDED=1`, each of the three user-only operations exits 2 with its hint; a cap set
   to an equal or lower value exits 0; and each operation exits 0 without the variable.
9. A `runner.env` entry for `TP_UNATTENDED` does not reach the child.
10. An escalating unit stops the run with artifacts intact, is not counted as a unit failure, sets no
    `last_failure`, and invokes `notify_cmd` when configured. An exit 2 with no escalation record is
    a unit failure instead.
11. An empty `next_units` on a non-releasable cycle stops the run with `no-units` on the first
    iteration rather than re-polling.
12. `tp run --dry-run` spawns nothing and lists the units that would run.
13. `tp run --status` reports an in-flight run, a converged run and each stopped run.
14. Runner templates resolve every placeholder, and an unresolved placeholder is a usage error before
    any child is spawned.
15. Spend is read from the runner's final log line; a runner declaring no `spend_key` reports `spend`
    null and leaves `cap-budget` inert.
16. A unit is attempted `1 + run_max_unit_retries` times.
17. The plugin validates with `claude plugin validate`, and its hook payloads are asserted against
    fixtures.
18. The `PreToolUse` deny covers `.tp-review/`, `*.tasks.json`, `.tp/config.json` and
    `.tp/local.json`, and does not fire for tp's own writes through a shell.
19. `.tp/run.json`, `.tp/runs/` and `.tp/last_failure.json` are git-ignored by the shipped
    `.tp/.gitignore`.
20. `location_clusters` groups a location reported by two roles under different classes, and leaves
    the recorded finding count, the `clean` flag and the `--status --check` exit code unchanged.
21. `next_action`'s mechanize branch carries the phase qualifier.
22. A file set truncated by the cap reports `truncated` true and `total_changed` above
    `total_files`, under `--quiet` as well.
23. An arity violation exits 2 for every command, including cobra's own built-ins, and `tp show`
