...
`TP_RUN_ID` is a ULID. `TP_RUN_DIR` is the absolute path of `.tp/runs/$TP_RUN_ID`, and
`TP_ROUND_DIR` is `$TP_RUN_DIR/<phase>-r$TP_ROUND`, so a round's role files sit in their own
directory and a record unit merges only its own round. `TP_FILE` is the resolved task file, so a
child works the same target the driver resolved rather than re-running discovery. Children are
...
| `review-resolve` | The spec revised and each addressed finding resolved | The spec's hash differs from `spec_hash`, or every finding is resolved | Alone |
...
| `audit-fix` | The code change for one audit finding committed | `HEAD` differs from `head_sha`, or the finding is recorded resolved without a change | Alone |
...
When one checkpoint satisfies several rows, the recorded reason is the first of `escalation`,
`unit-failure`, `interrupted`, `cap-budget`, `cap-wall-clock`, `cap-units`, `no-units`, `converged`.
...
unit's exit code and log path, and `stop_reason` when the run has ended. A run with no `stop_reason`
reports `in_flight` when `.tp/locks/run-<base>.lock` is held and `crashed` when it is not, which is
the only evidence that separates the two. On a stop in the audit
...
`{path, parsed, skipped}`, and an input whose lines all fail to parse makes `--merge` exit 1. The
same rule applies to both merges, because an unattended driver reads the exit code alone.
...
32. `--merge` reports `inputs` with per-file parsed and skipped counts, and exits 1 when an input's
    lines all fail to parse — for both `tp review --merge` and `tp audit --merge`.
...
35. `tp run --status` distinguishes an in-flight run from a crashed one by the run lock.
