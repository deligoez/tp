...
No other kind has a file to delete. A leftover therefore either means the role is
...
`review-resolve` and `audit-fix` add, so a re-run record unit re-records from it rather than
merging over it.
...
57. A re-run record unit preserves the dispositions already written into `merged.ndjson`.
