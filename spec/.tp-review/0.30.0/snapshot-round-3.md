...
10.9 **Rounds recorded before this release keep their positional ids.** An item id is legacy (positional) iff its final `-`-delimited segment is entirely digits — `file-go-safety-2` (final segment `2`) is legacy; `file-go-safety-config-validation` (final segment `config-validation`) is current. tp does not rewrite recorded round files, so a project mid-loop when it upgrades holds rounds whose ids follow the old scheme. Those rounds are read without error, and a prior-round section built from one states that its ids are positional and not comparable to the current round's — an honest gap is better than a false match between two ids that mean different things.
