...
10.9 **Rounds recorded before this release keep their positional ids.** Because tp knows the role that emitted the id, it strips the known prefix `file-<role>-` and classifies the id as legacy (positional) iff the remainder is entirely digits: `file-go-safety-2` (remainder `2`) is legacy, while `file-go-safety-validation-2` (a current collision suffix; remainder `validation-2`) and `file-go-safety-config-validation` (remainder `config-validation`) are current. The prefix-stripping rule therefore never misclassifies a current collision-suffixed id (`-2`, `-3`, …) or a slug that happens to end in a number as legacy. tp does not rewrite recorded round files, so a project mid-loop when it upgrades holds rounds whose ids follow the old scheme. Those rounds are read without error, and a prior-round section built from one states that its ids are positional and not comparable to the current round's — an honest gap is better than a false match between two ids that mean different things.
