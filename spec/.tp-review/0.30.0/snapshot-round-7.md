...
10.9 **Rounds recorded before this release keep their positional ids.** Every audit round tp records under v0.30.0+ carries an `id_scheme` marker — value `"slug"` — stating which id scheme its rows use. tp does not rewrite recorded round files, so a project mid-loop when it upgrades holds audit rounds recorded before this release that carry no marker: those are legacy rounds, whose rows use the old positional ids (`file-<role>-<n>`). The two id shapes are distinct — a positional id like `file-go-safety-2` names a row by its ordinal in that round, while a slug id like `file-go-safety-config-validation` names it by its subject — and detection is by the stored marker, not by parsing the id. A prior-round section built from a marker-less (legacy) round states that its ids are positional and not comparable to the current round's — an honest gap is better than a false match between two ids that mean different things. The `id_scheme` marker is recorded on audit rounds — the only rounds whose rows carry `(role, item_id)` and the only consumer; review rounds dedup on `(location, class)` and neither carry nor consume the marker. `tp audit --merge` deduplicates its input rows on `(role, item_id)`. Because v0.30.0 item ids are stable across rounds for the same subject (§10.3), merging rows from multiple slug-scheme rounds correctly deduplicates the same subject. Cross-scheme comparison — a legacy positional id against a current slug id — is the one case that would be unsound: the `id_scheme` marker flags legacy (marker-less) rounds, and a prior-round section built from one states its ids are positional and not comparable, so tp never silently matches a positional id against a slug id.
