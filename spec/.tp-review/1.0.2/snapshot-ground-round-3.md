...
> **This file is decisions. Its measurements are in `spec/1.0.1-measurements.md`, which `tp ground`
...
Precedent, derivation and the corrected count are in `spec/1.0.1-measurements.md` §1.
...
because the dispositions were written after the tag — is `spec/1.0.1-measurements.md` §2.1–§2.3.
...
construction. Two counters cannot do this — `spec/1.0.1-measurements.md` §2.4 gives the round that
...
transcript is in `spec/1.0.1-measurements.md` §3.
...
in `spec/1.0.1-measurements.md` §4.
...
unit whose brief told it to construct a counter-example. `spec/1.0.1-measurements.md` §4 has both, with
their transcripts.
...
accepting *some* of a round's findings does not clear it. The exit is `--resolve-all` with a shared
...
construction. `spec/1.0.1-measurements.md` §5 ranks them.
...
6. **The review phase's count is unchanged, and the reason is the corpus rather than the code.** §2's
   ...
   a guarantee. The count and its command are in `spec/1.0.1-measurements.md`.
...
`spec/1.0.1-measurements.md` §7 rather than here, because the quoted ones predated a test
