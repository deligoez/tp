...
4.1 `tp review` and `tp audit` change meaning by mode, and the spec positional belongs only to the modes that address spec state. In `--merge` and `--resolve`/`--resolve-all` modes **any positional argument is rejected**: tp accepts exactly zero positionals in those modes and exits 2 with a usage error naming the mode's expected argument shape (the error distinguishes 'you passed a spec positional to --merge' from an unknown flag). Today `tp audit <spec> --merge a.ndjson b.ndjson` silently parses the spec as an input file (reporting `input_files: 3` and one warning per spec line), and `tp review <spec> --resolve f.ndjson 0 fixed "why"` reads the index as the disposition.
...
5.1a **Commit-then-close.** tp stages and commits the task file still in its pre-close (`wip`) state, reads the implementation commit's sha (`C1`), then writes the closure record (`status: done`, `closed_at`, `gate_passed_at`, `commit_sha`/`commit_shas` = `C1`) into the task file.

5.1b **Fold via amend, guarded.** tp runs `git commit --amend --no-edit` to fold the closed task file into `C1`, producing the closure commit `C2` whose tree carries the done state. Both guards must hold: (i) `HEAD` is still `C1` (no commit landed between tp's commit and the amend), and (ii) `git diff --name-only HEAD` (working tree vs HEAD) lists only tp-owned paths written during this close — the task file plus any `.tp-review/` or `.tp/` state tp itself wrote in the same command. A non-tp-owned path differing fails guard (ii); this is what makes §2.2's clean-tree promise hold for all tp-owned state, not only the task file.

5.1c **`commit_sha` records the pre-amend implementation sha (`C1`), never the post-amend object sha.** A commit cannot contain its own object sha, so the task file carries `C1` — the implementation commit whose tree holds the audited source files — and the containing commit `C2` differs from it by exactly the folded closure record. `commit_sha`/`commit_shas[0]` is therefore `C1`; §11.1's `suggested_files` reads `C1`'s tree. `C2` is the clean-tree checkpoint; it is recorded nowhere on the task.
