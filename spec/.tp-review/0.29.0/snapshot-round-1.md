...
3.1 **Empty is a result, not an error.** `tp review --merge` and `tp audit --merge` succeed when the inputs contain zero findings: exit 0, an empty output file at `-o` (created, zero bytes), no finding lines on stdout, and a summary whose `merged_count` is 0. Findings stream to the `-o` file when given, else to stdout (one JSON object per line); the summary is a single JSON object on stdout when `--json` is set or stdout is piped, otherwise on stderr. The merge → record chain therefore works unchanged on a clean round.

3.2 **Failure stays failure.** Only genuinely unusable input fails: a missing or unreadable input file exits 3, and zero input files exits 2. A line is **malformed** if it is not valid JSON, and **incomplete** if it is valid JSON missing any of the required finding fields — `role`, `location`, `severity`, `finding`; both malformed and incomplete lines are skipped with a stderr warning (whose text names which) and never change the exit code.
...
| Input paths given but none readable | 3 | not written | error object |
...
4.1 `tp review` and `tp audit` change meaning by mode, and the spec positional belongs only to the modes that address spec state. In `--merge` and `--resolve`/`--resolve-all` modes the spec positional is **never** consumed as data: `--merge` takes only its explicit input-file arguments (a spec path among them is rejected, exit 2); `--resolve`/`--resolve-all` take the findings NDJSON file as the positional — `tp review <findings.ndjson> --resolve <idx> <disposition> "evidence"` and `tp review <findings.ndjson> --resolve-all <disposition> "evidence"` — and a `.md`/spec-looking positional where the findings file is expected exits 2 naming the expected form. Today `tp audit <spec> --merge a.ndjson b.ndjson` silently parses the spec as an input file (reporting `input_files: 3` and one warning per spec line), and `tp review <spec> --resolve f.ndjson 0 fixed "why"` reads the index as the disposition.
...
5.1 **A tp-made commit contains the closure in its final state.** Under `commit_strategy: builtin`, `tp commit` and `tp done --auto-commit` fold the task file's closure record into the implementation commit so the committed tree shows `status: done`. The mechanism:

5.1a **Commit-then-close.** tp stages the implementation files (selected via `--files` or auto-detected, as today) together with the task file still in its pre-close (`wip`) state, commits them as the implementation commit, and reads its sha (`C1`), then writes the closure record (`status: done`, `closed_at`, `gate_passed_at`, `commit_sha`/`commit_shas` = `C1`) into the task file.

5.1b **Fold via amend, guarded.** tp runs `git commit --amend --no-edit` to fold the closed task file into `C1`, producing the closure commit `C2` whose tree carries the done state. Both guards must hold: (i) `HEAD` is still `C1` (no commit landed between tp's commit and the amend), and (ii) `git diff --name-only HEAD` (working tree vs HEAD) lists only paths tp itself wrote during this command (the task file, plus any `.tp-review/` or `.tp/` state tp wrote in the same invocation). tp tracks the paths it wrote during the command, so a tp-owned path that was already dirty before tp ran (e.g. a hand-edited `.tp/config.json`) fails guard (ii) and falls back to §5.1d rather than being silently folded. A non-tp-owned path differing likewise fails guard (ii); this is what makes §2.2's clean-tree promise hold for all tp-owned state, not only the task file.

5.1c **`commit_sha` records the pre-amend implementation sha (`C1`), never the post-amend object sha.** A commit cannot contain its own object sha, so the task file carries `C1` — the implementation commit whose tree holds the audited source files — and the containing commit `C2` differs from it by exactly the folded closure record. `commit_sha`/`commit_shas[0]` is therefore `C1`; §11.1's `suggested_files` reads the paths `C1` touched (its diff, not its full tree). `C2` is the clean-tree checkpoint; it is recorded nowhere on the task.

5.1d **Follow-up fallback.** If either guard fails, tp creates a separate follow-up commit `chore(tp): record <id> closure` carrying the closed task file (and any tp-owned close state), leaving `C1` as the recorded `commit_sha`. Either path leaves `git status` clean for tp-owned paths when the command returns.

5.2 **Under `hc`, tp classifies instead of committing.** tp never runs `hc`, so `tp done --commit <sha>` legitimately leaves tp-owned files modified. `tp resume` gains a `bookkeeping` array — uncommitted files that are tp's own state (`<base>.tasks.json`, anything under `.tp-review/`, anything under `.tp/`) — reported separately from `changes` and never counted as an `unexplained-changes` blocker. Each entry is `{path, kind, ref}`: `path` is repo-root-relative; `kind` is `closure` (the task file; `ref` = task id), `round` (a `.tp-review/` round or snapshot file; `ref` = round number from the filename), or `config` (any other dirty `.tp/` state, e.g. `.tp/config.json` or `.tp/.gitignore`; `ref` = path basename). tp derives `kind`/`ref` by diffing the path against `HEAD`: for the task file it emits one `closure` entry per task whose managed fields (`status`, `closed_at`, `gate_passed_at`, `commit_sha`) changed since HEAD; for a `.tp-review/` file it reads the round number from the filename. The array is `[]` when no tp-owned file is dirty. `next_action` is unaffected; the summary names the pending bookkeeping so the agent commits it with the project's commit tool.
...
In the §6.1 table, **blank** means empty after trimming whitespace, applied identically to `id`, `title`, and `acceptance`; `id` comparison is exact, case-sensitive string equality. In `--bulk`/`--stdin` batches validation runs after the whole batch is staged, so an entry may `depends_on` a task appearing later in the same batch. `tp add` applies exactly these seven rules, and `tp import` already enforces the same set, so the table is a restatement rather than a divergence.
...
7.1 `AutoFillCoverage` runs after any command that changes the task set or a task's anchors — `add`, `import`, `remove`, and `set` of `source_sections`/`source_lines` — whenever the spec file is readable. Today only `tp import` calls it, so the `tp init` + `tp add` path documented in the project's own QA setup produces a file that fails `tp validate` with `total_sections is 0 but spec has N headings` and no way to repair it short of a round trip through `import`. Status-only writes (`claim`, `close`, `done`, `reopen`) change no anchor or task-set shape, so they skip the recompute.
...
8.4 **Compact disposition of the new output fields.** `--compact` is tp's token-minimizing mode (P3), so each new field states whether it survives: decision-critical fields stay under `--compact` — `bookkeeping` (the agent must commit them), `suggested_files` (the agent branches on them to select audit targets), and `max_rounds`/`rounds_remaining`/`in_flight_round` (a driver branches on remaining budget); explanatory fields are omitted under `--compact` — `skipped_roles`, `attribution_excludes`, the audit `overlap_report`, and the report `note`. This mirrors §8.3's treatment of `spec_excerpt`.

## 9. Role and attribution transparency

9.1 **A corpus role that produces no prompt is named.** `tp review` and `tp audit` prompt emission adds `skipped_roles`: an array of `{role, reason}` covering every corpus role that was not emitted, with `reason` one of `no-checklist-items` (role focus/checklist empty), `no-spec-change` (`--diff-from` active and the role's focus is outside the changed sections), `domain-mismatch` (role domain ≠ file domain), or `no-baseline` (the regression role at round 1, which has no `snapshot-round-0.md`). The array is `[]` when no role was skipped. In the QA run the software auditor corpus ships three roles and tp emitted two, with nothing to distinguish an intentional skip from a misconfigured corpus.

9.2 **Attribution exclusions are stated when they bite.** `found_by_roles` deliberately excludes the built-in `regression` role, so a finding contributed only by regression carries no roles and vanishes from the overlap report while still counting in `merged_count`. The merge summary and `--status` output add `attribution_excludes: ["regression"]` only when that exclusion actually causes `merged_count` to exceed the overlap-report finding count — the single scenario where the absence would otherwise trigger investigation; when the counts already match the field is omitted (and it is always omitted under `--compact`, see §8.4).
...
10.2 **An interrupted round is visible.** `.tp-review/<base>/` holds both `snapshot-round-N.md` and `review-round-N.ndjson`; a snapshot with no round file means a round was started and never recorded. tp writes `snapshot-round-N.md` when round N begins (prompt emission) and `review-round-N.ndjson` when round N is recorded (`--record`); the snapshot write is atomic (write to `snapshot-round-N.md.tmp` then rename), so a partial snapshot is never left on disk. `--status` reports `in_flight_round: N` (`null` when none), and `tp resume`'s `next_action` for that state is `{"action":"record-round","round":N}` pointing at completing and recording round N rather than starting a new one.

10.3 **The regression prompt carries its baseline path.** The regression role's prompt names the snapshot it diffs against (`.tp-review/<base>/snapshot-round-<N-1>.md`), so a fresh sub-agent can read the baseline without the orchestrator injecting the path. For round 1 there is no `snapshot-round-0.md`; the regression role is added to `skipped_roles` with reason `no-baseline` rather than emitted with an empty diff.

## 11. Audit file selection

11.1 When `tp audit` finds no audit-able file in the diff it exits 4, and the hint names `--base`/`--affected-files` without naming any file — leaving a fresh agent to derive the list from git itself. The exit-4 payload gains `suggested_files`: the union of paths touched by the commits recorded in `commit_shas` of every task with `status: done` in the active file (every SHA in the array, including reopened-then-redone tasks' history), with the same type filtering the auto-detection applies. When `suggested_files` is empty (e.g. a task closed via `--covered-by`, or a legacy close with no `commit_shas`), `tp audit` still exits 4 with `suggested_files: []` and a hint naming `--affected-files`/`--affected-from-tasks`.
...
13.2 Every error object tp emits on a non-zero exit (validation, usage, file, or state) carries a `hint` that names the next command to run, not only the rule that was broken.

## 14. Report accuracy

14.1 `tp report` divides the estimate by the measured duration whenever it is greater than zero, so a task closed within microseconds yields `actual_minutes: 0` beside `accuracy: 300000000`. `actual_minutes` is rounded to one decimal place; when it rounds to `0.0`, `accuracy` is `null` and the task carries `note: "duration below resolution"`.
...
16.2 The project default becomes settable: `tp set --workflow --project commit_strategy=<builtin|auto|hc>` writes `workflow.commit_strategy` in `.tp/config.json` and participates in the existing resolve-at-read order. At close time `auto` resolves by the same rule as everywhere else — `hc` when `hc` is on `PATH`, else `builtin` — and the close then follows §5.1 (`builtin`) or §5.2 (`hc`) accordingly. The task-file value stays init-authored, and `tp set --workflow commit_strategy=…` keeps its current exit-2 refusal, with the hint now naming the project-level setter.
