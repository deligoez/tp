...
| 8 | a review role prompt is emitted | the key list in the prompt's output format — the JSON object line a reviewer copies — names `evidence` | every path in the emission-site list under *The review emission sites, derived* in `spec/1.1.0-measurements.md`, which derives the list from the tree rather than restating it; the check parses each path's JSON object line, and a substring search of the whole prompt is not the check | a listed path whose output format lacks the key — which is `HEAD`'s `--verify`, whose prompt carries no JSON object line at all |
...
| 10 | `tp review --merge` exits non-zero | leave no file at the `-o` path | two input files: the first holds one content line that parses as no finding — its only row omits `location` — so that input is dropped and the merge refuses; the second holds one row legal on all four keys; the `-o` path does not exist before the run | `HEAD`, which writes the surviving row to the `-o` path and then exits 1, so the path exists after the refusal |
