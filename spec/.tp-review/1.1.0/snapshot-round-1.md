...
| 8 | a review role prompt is emitted | the key list in the prompt's output format — the JSON object line a reviewer copies — names `evidence`, **and every site in the prompt that names keys names the same set** | any spec, `--role implementer`; the check parses each key-naming site and compares the sets, and a substring search of the whole prompt is not the check | an emission whose key-naming sites disagree with each other — which is `HEAD`, where three sites name three different sets and `evidence` already occurs in the surrounding prose and in none of them, so a substring check is green with nothing built |
