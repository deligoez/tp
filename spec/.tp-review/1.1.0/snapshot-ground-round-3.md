...
| 5 | a row omits one of `severity`, `finding`, `location`, `evidence` | `--merge` skips it, naming the missing key in the stderr warning it already writes for the other three; `--record` exits 1 naming its line | the offending row **alone in its own input file**, so the skip is observable — that file then contributes zero rows — rather than masked by a legal row beside it in the same file; a second file holds one row legal on all four keys | `evidence` is added to `--record`'s check only and not to `missingFindingFields`, so `--merge` parses the offending row and the two sets diverge again |
...
| 8 | a review role prompt is emitted | the prompt's output contract names `evidence` among the keys every finding must carry | any spec, `--role implementer` | an emission that drops the key from the contract: absent |
