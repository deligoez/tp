...
are met here: §2 changes what `tp review --record` accepts, and §5 row 8 changes the review role
...
2. **`--merge` and `--record` require the same fields.** The set is `severity`, `finding`,
   `location` and `evidence`. `--merge` skips a row missing one of them with a warning on stderr;
   `--record` refuses it. Today the two diverge — `--record` requires none of the four — which is
   how a row holding only `{}` records as a finding at exit 0.
...
4. **The skill states what a spec is about.** A sentence that would have to change if the
   ...
   author.
5. **The release is sized by the fixtures it breaks, not by §5's rows.** Adding `evidence` to the
...
Its `review-record` unit command is in scope for the same reason: §5 row 10 changes what a refused
`--merge` leaves on disk, and that command chains the merge into `--record` with `;` behind an
`[ -f ]` guard, so it can walk a refused merge's leftovers into the record — which, after this
release, it must not be able to do.
The reference is also where this release's meaning of *whitespace* belongs, because §2's predicate has
one and no acceptance row can pin its boundary: the whitespace `strings.TrimSpace` recognises, and
nothing outside that set.

`skills/tp/SKILL.md` is in scope beyond the four rules, and for the same reason as the reference: the
merge step of its review loop states §5 row 10's rule as **already-shipped fact** — that `-o` writes
only after the merge succeeds, so a failed merge leaves nothing behind — and that sentence is false
at `HEAD`.
...
- **No change to the audit phase.** Audit rows already carry `evidence_file` and `evidence_lines`.
  The audit side of the acceptance channel is `spec/backlog/a-finding-can-leave-an-audit-round.md`.
...
|---|---|---|---|---|
| 1 | a findings file handed to `tp review <spec> --record` holds rows missing a required field | exit 1 naming **every** offending line, write no round file, leave `review_rounds` unchanged | a four-row file, one row per required key: the first omits `severity`, the second `finding`, the third `location`, the fourth `evidence`; each row is legal on the other three, and it is recorded against a spec that **already holds one recorded round**, so `review_rounds` has a non-zero value to be unchanged from | a record-side check that tests only `evidence`. It still exits 1 and still names the fourth line, so the exit code does not separate the two — what separates them is the **set** of lines named, and the three rows missing the other keys go unnamed |
| 2 | the file's only row is `{}` | exit 1, write no round file, leave `review_rounds` unchanged | a one-line file holding `{}`, recorded against a spec that **already holds one recorded round**, so `review_rounds` has a non-zero value to be unchanged from — at `HEAD` this records at exit 0 as one finding and advances the round, which is the defect this row closes | a check that reads only the keys a row actually has, so a row with no keys at all passes it |
| 3 | every row carries `severity`, `finding`, `location` and a non-empty `evidence` | record the round with `findings: 3` and `consecutive_clean: 0` | a three-row file, legal on all four keys and carrying no `resolved` block, its three severities `high`, `medium` and `low`, recorded against a spec with no prior round; no row's `evidence` length is load-bearing here and any non-empty text serves | `--record` tests `resolved.evidence` — the key the record path already reads — in place of the top-level `evidence`, so every row of the legal file is refused: exit 1, no round written |
...
| 7 | `tp review <spec> --status` reads a round recorded before this release, whose rows carry no `evidence` | exit 0 with `consecutive_clean: 0` (exit 1 under `--check`, as before) — the requirement is enforced when a round is recorded, never when one is loaded | a pre-`1.1.0` round file copied under `testdata` holding one undispositioned `high` row and no `evidence` key — not the live corpus, which stops being uniformly pre-`1.1.0` at this release's own first recorded round | a status path that applies the record-time check on load: exit 1 |
| 8 | a review role prompt is emitted | the key list in the prompt's output format — the JSON object line a reviewer copies — names `evidence` | the five commands that emit a finding contract, named here rather than counted: `tp review <spec>`, `tp review <spec> --role <r>`, `tp review <spec> --perspective code-audit`, `tp review <spec> --perspective regression` and `tp review <spec> --verify`. Each is emitted and its output-format object line parsed; a substring search of the whole prompt is **not** the check, because the word already occurs in a panel prompt's instruction text and in whatever an `--affected-files` excerpt happens to contain | a listed command whose output format lacks the key — which is `HEAD`'s `--verify`, whose prompt carries no JSON object line at all |
...
| 10 | `tp review --merge` exits non-zero | create no file at the `-o` path, and modify no file already there | two input files — the first's only row omits `location`, so that input parses no row and the merge refuses at exit 1; the second is legal on all four keys — run three times against one `-o` path: once with the path absent, once with it already holding a seed line whose bytes are pinned before the run, and once with a missing input file named in place of the first, which refuses at exit **3** against the same seeded path | `HEAD`, which writes the surviving row to `-o` *before* it refuses: after the first run the path exists, and after the second the seed is gone and the file holds the merged row instead. The exit-3 run is already compliant at `HEAD` and is in the fixture so that the SHALL's "non-zero" is pinned by more than exit 1 |
| 11 | a row's `location` is `" "` — present, a non-empty string, empty once trimmed | `--merge` skips it with its stderr warning naming `location`, exactly as for a missing `location` | one file, two rows — the first carries `"location": " "` and is legal on the other three keys, the second is legal on all four, so the file still parses one row and the skip shows as a warning rather than as a dropped input | `HEAD`'s `missingFindingFields`, which tests `x == ""`: the file parses two rows, no warning is written, and `--merge` exits 0 |
