...
17. **Round budgets**: optional `review_max_rounds` / `audit_max_rounds` caps that escalate to the user when exhausted, without ever bypassing import enforcement (§8.8).
...
The task-to-file mapping: a task maps to the files changed by its recorded `commit_sha` (`git show --name-only <sha>`), intersected with the files present in `git diff base..HEAD`. Tasks without a `commit_sha`, or whose sha is unknown to git, map to zero files. This mapping is the single source of file paths derived from tasks (also used by §3.3). Task ids within a file's task list and file paths within a task's mapped-file list are sorted alphabetically; the §5.4 annotations, §5.6 `tasks` arrays, and §3.3 comma-separated paths inherit this order, extending the §3.2 regeneration-determinism contract from item ids to the ordering of ids and paths throughout the prompt body.
...
A role whose checklist is empty — including a file-checklist role (`security`, `maintainability-conventions`) whose file selection is empty — is omitted from `prompts`. `tp audit` emits one prompt per role that has at least one checklist item: 3 prompts when all roles are populated. Prompts appear in the fixed order `spec-coverage`, `security`, `maintainability-conventions`, with omitted roles skipped.
...
3. Previous findings: all rows from `review-round-1..R-1.ndjson`, deduplicated by the existing finding identity key, feed the findings summary injected into every role prompt (implementer, tester, architect); the regression prompt carries no findings summary — its body is §11.3's blocks plus §12.3 lens injection and, when checks are registered, the §15.5 mechanical-exclusion block. An explicit `--findings` flag overrides this source.
...
State participation by perspective: the default (three-role) mode reads and writes state as above; `--perspective regression` standalone reads state only in its state-reading mode (§11.1 mode (a)) and never writes it — no snapshot, no `state.json` change; with explicit `--diff-from` plus `--findings` it neither reads nor writes state; `documentation`, `testing`, and `code-audit` neither read nor write state. There are no other perspective values.
...
2. The file is copied to `review-round-<R>.ndjson` (R = recorded rounds + 1) and a round entry `{round, findings, clean, recorded_at, file, spec_hash}` is appended to `state.json`, where `findings` = row count, `clean` = (count of rows without `resolved.status == "wontfix"` == 0), and `spec_hash` = SHA-256 of the current spec bytes. A row MAY arrive pre-resolved `wontfix` when the §16 tail-protocol verification pass rejected it; such rows do not dirty the round. `--record` rejects a pre-resolved `wontfix` row whose `resolved.evidence` is empty or whitespace-only (exit 1 with the line number); whether the evidence names a verifier is SKILL.md convention, not tp-validated. A row pre-resolved `duplicate` follows the default rule and dirties the round — dedup belongs in `--merge`, before recording. A row arriving pre-resolved `fixed` aborts the record with exit 1 (a fix means the spec changed; the staleness rule surfaces that). `--record` creates the state directory when absent; it never writes snapshots. A round-budget refusal (§8.8) is evaluated first — before line parsing, row validation, and any state write.
3. Rounds are immutable once recorded; later `--resolve` edits to the round file change finding resolution status but never the recorded count or the recorded `clean` flag — both are computed once at record time.
...
- `--status` prints `{review_rounds, consecutive_clean, required_clean_rounds, converged, stale, mechanical_checks}` — plus `budget_exhausted` iff a review cap is set (§8.8). Without `--check`, `mechanical_checks` lists the registered `{class, cmd}` pairs with no execution results. When no state directory exists or the round index is empty, `--status` prints the full shape — `{review_rounds: [], consecutive_clean: 0, required_clean_rounds: <resolved per §8.6>, converged: false, stale: false, mechanical_checks: <registered list>}` — plus `budget_exhausted: false` iff a cap is set — with exit 0.
...
- Workflow resolution for every review/audit state command (`review_clean_rounds`, `audit_clean_rounds`, `review_max_rounds`, `audit_max_rounds`, `gate_timeout_seconds`, `checks`): the task file is resolved via the standard discovery chain (`--file` > `TP_FILE` > `.tp-active` > auto-detect); when the discovered file's `spec` field does not resolve to the spec under review (both paths cleaned and made absolute — the `spec` field against the task file's directory, the CLI argument against the CWD — then compared for equality), or no file is found, the spec-adjacent `<spec-base>.tasks.json` is used; when neither exists, defaults apply (2 clean rounds, no cap, 600s timeout, no checks). In the §16 workflow, `tp init <spec>` creates the spec-adjacent file before the review loop, so the file the loop reads and the eventual import target are the same file.
...
| `convergence` | "no new high-severity findings" | "no findings surviving verification (any severity) in `<N>` consecutive rounds" |
...
When `review_max_rounds` > 0, recorded review rounds ≥ the cap, and `converged` is false, `tp review` (default mode) refuses to generate further prompts: exit 4 with a hint naming the ways forward — exhaust the tail with uncounted passes (§11.2 delta pass, §16 item 12 class sweep) and then, with user approval, raise the cap via `tp set --workflow` for the confirming full-panel rounds, or import with user-approved `--force`. `--record` appends rounds up to the cap; while `converged` is false it refuses to append a round entry whose number would exceed the cap (exit 4, same escalation hint), so self-recorded clean rounds cannot tunnel past an exhausted budget — the user-approved cap raise and `--force` are the only exits. `budget_exhausted` = (cap > 0) AND (recorded rounds ≥ cap) AND NOT `converged`; the field appears in `--status` output only (review and audit shapes, §8.5/§10.2, including their empty-state shapes) and only when the governing cap is > 0 — the `--record` output schema (§8.4) is unchanged. The cap never changes import enforcement (§9.1): an unconverged spec stays blocked regardless of budget. Audit prompt generation, `tp audit --record` (same beyond-cap refusal while audit `converged` is false), and `tp audit --status` mirror the rule with `audit_max_rounds`; cap-triggered audit generation reads state and inherits the §8.2 corrupt-state abort.

## 9. Import Convergence Enforcement

### 9.1 Enforcement rule

`tp import` locates the state directory for the resolved spec path — a bare-array `--spec` value resolves against the CWD; a wrapped payload's `spec` field resolves against the import target task file's directory (matching how §8.6 reads it after the write). When the directory exists and contains at least one recorded review round, two conditions are checked: (a) `consecutive_clean >= required`, where `required` = the workflow `review_clean_rounds` value that will govern the imported file (imported payload's value, or the preserved existing value per §9.3, or default 2) — when this differs from the §8.6-resolved value that `--status` used, import emits a warning naming both values; (b) the state is not stale — the current spec's SHA-256 equals the last recorded round's `spec_hash` (§8.6). A failed condition fails the import with exit 1 ("review not converged: <X> consecutive clean rounds, <required> required" or "spec changed since round <N> was recorded") plus a hint naming `tp review --record` and `--force`; when the round budget is exhausted the hint instead names the §8.8 escalation sequence — user-approved cap raise, then `tp review --record` for the confirming rounds, or user-approved `--force`. The existing `--force` flag bypasses both checks. Importing over an existing task file that contains zero tasks (the `tp init` shell) does not require `--force`, so the canonical Workflow A ends with a plain `tp import` and the convergence checks stay armed; `--force` is reserved for overwriting a file with real tasks.
...
3. The file is copied to `audit-round-<N>.ndjson` (N = recorded audit rounds + 1, a sequence independent of review rounds) and appended to `state.json.audit_rounds`. A round file may contain rows from every role prompt of the audit; role-prefixed `file_check` ids (§3.2) keep rows attributable. The §8.8 round-budget refusal applies to audit `--record` identically.
...
Output shape: `{audit_rounds, consecutive_clean, required_clean_rounds, converged, stale}` — plus `budget_exhausted` iff `audit_max_rounds` > 0 (§8.8); no `mechanical_checks` field, because audit `--check` does not run `workflow.checks` (they guard review rounds). `--check` exits 0 only when audit `converged` is true and requires `--status`. Audit `--record` and `--status` are mutually exclusive with each other and reject `--affected-files` and `--findings` (exit 2). When no state directory exists or the audit round index is empty, `--status` prints `{audit_rounds: [], consecutive_clean: 0, required_clean_rounds: <resolved per §8.6>, converged: false, stale: false}` — plus `budget_exhausted: false` iff a cap is set — with exit 0.

### 10.3 Scope note

Aside from the §8.8 round-budget refusal, `--record` performs row counting only — it validates JSON well-formedness and reads the `status` field. It never reads `class`, and its output omits `mechanize_candidates`. Schema validation of §4.2 field requirements belongs to the deferred `tp audit --merge`.
...
1. Workflow A order becomes: interview → `tp lint` → `tp init` (with `--quality-gate` for the gate command) → `tp set --workflow` (convergence parameters, round budgets `review_max_rounds` / `audit_max_rounds`, `checks`) → review loop → decompose → `tp import` (plain — the init shell holds zero tasks, so the overwrite needs no `--force` and the §9.1 convergence checks stay armed). Running `tp init` before the review loop creates the spec-adjacent task file that supplies the loop's workflow parameters (§8.6); the gate itself is authored at init time because `tp set` keeps it read-only (§6.7).
2. The review loop step becomes an explicit recipe: `tp review <spec>` → spawn sub-agents per prompt → merge findings (`--merge`) → `tp review <spec> --record <merged.ndjson>` → fix spec → resolve findings (`--resolve`) → run the standalone regression delta pass (§11.2) whenever the fix batch touched more than 3 sections → repeat until `tp review <spec> --status --check` exits 0.
...
4. A gate rule is added: the gate runs automatically at `tp done`; `--skip-gate` requires explicit user approval and is never the agent's own decision. The same approval rule covers budget exhaustion (§8.8): the agent stops and escalates; raising `review_max_rounds` / `audit_max_rounds` after exhaustion and a `--force` import are never the agent's own decision.
...
12. Tail protocol: when a round's findings drop to one or two low/medium items, (a) route each disputed finding through `tp review --verify`; a verifier-rejected finding is resolved `wontfix` with the verifier's reasoning and recorded with the round without dirtying it (§8.4) — the wontfix resolution is written into the findings file before `--record`, because the round entry never recomputes; (b) derive the class of each surviving tail finding and run one exhaustive class-sweep prompt per class ("enumerate every <pattern> in the spec; verify each") before the next counted round, so a single class cannot drip one finding per round.
...
| Workflow JSON | — | new fields `gate_timeout_seconds`, `checks`, `review_max_rounds`, `audit_max_rounds` |
...
1. **Model fields** — `Task.GateSkippedReason`; `Workflow.GateTimeoutSeconds`, `Workflow.Checks`, `Workflow.ReviewMaxRounds`, `Workflow.AuditMaxRounds` with unmarshal defaults and validation.
...
- `TestReviewRecord_CleanAndDirty`: empty file → clean round; whitespace-only lines skipped; parse error → exit 1 with line number; a file whose rows are all pre-resolved `wontfix` records a clean round; a pre-resolved `fixed` row → exit 1; a pre-resolved `wontfix` row with empty evidence → exit 1; a pre-resolved `duplicate` row dirties the round
...
- `TestRoundBudget_Escalates`: `review_max_rounds=2` and `audit_max_rounds=2`, each with 2 dirty rounds recorded → review prompt generation exits 4 with the escalation hint; a further `--record` (review or audit, beyond the exhausted cap) is refused with exit 4; `--status` reports `budget_exhausted: true`; import enforcement unchanged; `audit_max_rounds` blocks audit prompt generation identically and `tp audit --status` reports the field
...
- `tp review --record` appends an immutable round entry with `spec_hash` and reports `consecutive_clean`, `converged`, `stale`, and `mechanize_candidates`; verifier-rejected rows pre-resolved `wontfix` do not dirty the round, and a pre-resolved `fixed` row aborts
...
- Reaching `review_max_rounds` (or `audit_max_rounds`) without convergence blocks further prompt generation (exit 4); `--record` appends rounds up to the cap and refuses an entry beyond an exhausted cap (exit 4); `--status` reports `budget_exhausted`; the cap never relaxes import enforcement
...
- `review_loop` output carries `required_clean_rounds`, `consecutive_clean`, `converged`, and the no-findings-surviving-verification convergence text; `max_rounds` is gone
