...
Item ids are deterministic for a given spec + task file + diff: spec-derived ids follow the existing `table-<t>-<r>` / `list-<l>-<n>` / `task-<id>` schemes; `file_check` items use `file-<n>` where `<n>` is the item's zero-based index in the role's affected-files list. Regenerating prompts without changing spec, tasks, or diff yields identical ids.
...
- **`security`**: files in `git diff base..HEAD` whose **path** contains any case-insensitive substring from `{lock, validate, auth, secret, perm}` OR whose **first 200 lines in the working tree** contain any of those substrings (case-insensitive). Ranked alphabetical.
...
Format breakdown:
- Path (repo-relative)
- `(tasks: <comma-separated task ids>; diff: +N/-M)` annotation for `spec-coverage`
...
- Files deleted in `git diff base..HEAD` (no working-tree content to audit); renamed files enter under their new path only
...
6. Locking: the gate runs BEFORE the task-file flock is acquired. After the gate passes, tp acquires the flock, re-reads the task file, and re-validates every target task's state transition; a task whose state changed during the gate run fails with the normal state error.
...
3. Batch entries opt in per entry via a `skip_gate` string field in the NDJSON. When every entry in the batch carries `skip_gate`, the gate is not executed at all.
...
The state directory is meant to be committed to version control — import enforcement (§9) holds across clones and CI only when the recorded rounds travel with the repo. Only `state.json` and the newest snapshot are load-bearing: older `snapshot-round-<N>.md` files MAY be deleted at any time (the diff falls back per §8.3 item 4), and round NDJSON files MAY be pruned once resolved, at the cost of regression-prompt history (§11.3).
...
- Workflow resolution for every review/audit state command (`review_clean_rounds`, `audit_clean_rounds`, `gate_timeout_seconds`, `checks`): the task file is resolved via the standard discovery chain (`--file` > `TP_FILE` > `.tp-active` > auto-detect); when the discovered file's `spec` field does not name the spec under review, or no file is found, the spec-adjacent `<spec-base>.tasks.json` is used; when neither exists, defaults apply (2 clean rounds, 600s timeout, no checks). In the §16 workflow, `tp init <spec>` creates the spec-adjacent file before the review loop, so the file the loop reads and the eventual import target are the same file.
...
`tp review <spec> --perspective regression` requires either (a) a state directory with R ≥ 2, or (b) explicit `--diff-from <old-spec>` plus `--findings <file>`. Missing inputs are a usage error (exit 2). Standalone regression reads state (rounds, snapshots, resolved findings) but never writes it (§8.3).
...
1. Workflow A order becomes: interview → `tp lint` → `tp init` (with `--quality-gate` for the gate command) → `tp set --workflow` (convergence parameters, `checks`) → review loop → decompose → `tp import --force`. Running `tp init` before the review loop creates the spec-adjacent task file that supplies the loop's workflow parameters (§8.6); the gate itself is authored at init time because `tp set` keeps it read-only (§6.7).
...
6. **Engine: audit categories + file filtering** — `audit_categories.go` (enum + `ResolveAuditCategory`, which renders the §2.2 precedence rule embedded in every audit prompt's Role Rules and is the single enum source for the future `--merge`), `audit_files.go` (`FilterAffectedFilesForRole` per §5).
...
- `expected_evidence` populated per §3.3 (never null; non-empty for task items with a usable `commit_sha`)
...
- `tp import` fails on an unconverged or stale reviewed spec, proceeds with an info line when no rounds exist, and preserves the existing workflow block when the payload has none
