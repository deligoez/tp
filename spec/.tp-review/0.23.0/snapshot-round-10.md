...
2. The file is copied to `review-round-<R>.ndjson` (R = recorded rounds + 1) and a round entry `{round, findings, clean, recorded_at, file, spec_hash}` is appended to `state.json`, where `findings` = row count, `clean` = (count of rows without `resolved.status == "wontfix"` == 0), and `spec_hash` = SHA-256 of the current spec bytes. A row MAY arrive pre-resolved `wontfix` when the §16 tail-protocol verification pass rejected it; its resolution evidence MUST name the verifier's reasoning, and such rows do not dirty the round. A row arriving pre-resolved `fixed` aborts the record with exit 1 (a fix means the spec changed; the staleness rule surfaces that). `--record` creates the state directory when absent; it never writes snapshots.
...
- `TestReviewRecord_CleanAndDirty`: empty file → clean round; whitespace-only lines skipped; parse error → exit 1 with line number; a file whose rows are all pre-resolved `wontfix` records a clean round; a pre-resolved `fixed` row → exit 1
