...
- **`security`**: files in `git diff base..HEAD` whose **path** contains any case-insensitive substring from `{lock, validate, auth, secret, perm}` OR whose **first 200 lines at the HEAD revision** contain any of those substrings (case-insensitive); files absent at HEAD (new or untracked entries of an `--affected-files` universe) fall back to working-tree content for this heuristic, and files unreadable in both places are judged by the path heuristic alone. Ranked alphabetical.
