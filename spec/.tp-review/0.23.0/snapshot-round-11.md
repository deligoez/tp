...
When `review_max_rounds` > 0, recorded review rounds ≥ the cap, and `converged` is false, `tp review` (default mode) refuses to generate further prompts: exit 4 with a hint naming the ways forward — exhaust the tail with uncounted passes (§11.2 delta pass, §16 item 12 class sweep) and then raise the cap via `tp set --workflow` for the confirming full-panel rounds, or import with user-approved `--force`. Convergence still requires counted rounds whose prompts tp generated; raising the cap and `--force` are the only exits. `--record` keeps recording. `budget_exhausted` = (cap > 0) AND (recorded rounds ≥ cap) AND NOT `converged`; the field appears in `--status` output only (review and audit shapes, §8.5/§10.2, including their empty-state shapes) and only when the governing cap is > 0 — the `--record` output schema (§8.4) is unchanged. The cap never changes import enforcement (§9.1): an unconverged spec stays blocked regardless of budget. Audit prompt generation and `tp audit --status` mirror the rule with `audit_max_rounds`; cap-triggered audit generation reads state and inherits the §8.2 corrupt-state abort.
...
3. The file is copied to `audit-round-<N>.ndjson` (N = recorded audit rounds + 1, a sequence independent of review rounds) and appended to `state.json.audit_rounds`. A round file may contain rows from every role prompt of the audit; role-prefixed `file_check` ids (§3.2) keep rows attributable.
...
4. A gate rule is added: the gate runs automatically at `tp done`; `--skip-gate` requires explicit user approval and is never the agent's own decision. The same approval rule covers budget exhaustion (§8.8): the agent stops and escalates; a `--force` import is never the agent's own decision.
...
- `TestRoundBudget_Escalates`: `review_max_rounds=2` with 2 dirty rounds recorded → review prompt generation exits 4 with the escalation hint; `--record` keeps recording and `--status` reports `budget_exhausted: true`; import enforcement unchanged; `audit_max_rounds` blocks audit prompt generation identically and `tp audit --status` reports the field
...
- Reaching `review_max_rounds` (or `audit_max_rounds`) without convergence blocks further prompt generation (exit 4); `--record` keeps recording, `--status` reports `budget_exhausted`, and the cap never relaxes import enforcement
