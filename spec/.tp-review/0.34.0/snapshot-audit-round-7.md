...
`workflow.checks` run in the review phase only, while the list is written afterwards, so no review
round can see the list compared against the derivation. **That task's acceptance is therefore to run
...
check for a class seen in three rounds becomes conditional on the phase. A check registered where
the thing it measures does not yet exist suppresses a finding class for every reviewer while
verifying only whatever part of its subject already exists — which may be nothing, and was not
enough here. §7.1 records the instance; this is where the rule itself is corrected.
