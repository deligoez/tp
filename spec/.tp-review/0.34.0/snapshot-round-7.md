...
2. `CLAUDE.md` and `skills/tp/SKILL.md` currently instruct `tp init --quality-gate "…"`. Both recipes
   ...
   The flag stays documented in `skills/tp/REFERENCE.md` as the route for a project that does want a
   task-level gate; what changes is what the recipes recommend, not what exists.
