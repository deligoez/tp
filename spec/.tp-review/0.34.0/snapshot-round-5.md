...
this release's task file runs `-race`, and that a test fails if it stops doing so.** Which override
to remove, by what route, and what the assertion compares against are settled against the code at
decomposition; four routes have already been proposed in review and three of them were disproven by
measurement, which is the evidence that this is not a question prose can close.

One constraint does belong here, because it is a policy rather than a mechanism: the guard must not
compare the resolved value against the project value, since that comparison passes when the project
gate weakens.
...
touched between the `v0.32.0` and `v0.33.0` tags. That window is the evidence-driven one — three of
...
The outcome required is that **a test fails when the documented set and the emitted set diverge.**
Sourcing each side is decomposition's problem and it is not trivial: rules are emitted from more
than one package, so an enumeration owned by one of them cannot make absence imply non-emission, and
any construction that would guarantee it by typing is the refactor Non-Goal 7 excludes. A test that
compares two honestly-collected sets is worth more than a construction that cannot be built.
...
They are added to their canonical home, and the flag inventory gains the same kind of equality test:
it fails when a flag the CLI registers is undocumented. Note the interaction with §5 — a flag stays
in the inventory even when the documented workflow stops recommending it, because the inventory
records what exists, not what to use.
...
words. Four review rounds produced eleven blocking findings that were all instances of one shape:
the list and its section drifted, and each round's measurement overturned the previous round's
prose. The list is gone.

Each section above states the outcome that must become observably true. The assertion that proves
it — the exact recipe, the exact exit code per entry point, the exact set on each side of an
equality — is authored as task acceptance during decomposition, where it is settled by running the
code rather than by arguing about the text. This is tp's own `over-specification` class applied to
tp's own spec: a detail whose correctness can only be established against code belongs in
acceptance, not here.

Two rules govern that authoring, because both are policy rather than mechanism:

1. **Every assertion must fail on today's tree.** An assertion that already passes proves nothing
   about the change it accompanies. Verify by running it before the fix, not after.
