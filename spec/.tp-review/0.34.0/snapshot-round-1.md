...
**A task in this release must be able to name what it removed.** A task that only adds is not
forbidden, but it is the exception and it says so in its evidence.

This inverts the usual default deliberately. An agent asked to improve a codebase reliably adds: a
helper, a flag, a wrapper, a test. Each addition is locally defensible and the sum is the drift this
release exists to reverse. Removal is the direction that needs the explicit instruction because it
is the one nobody drifts toward.
...
`dupl` is enabled in `.golangci.yml` at a threshold tuned against the current tree, and the clones
it reports are either merged or exempted with a reason at the site. The threshold is chosen so the
initial report is small enough to act on; a detector whose first run produces a hundred findings
gets disabled instead of used.
...
Scope: every test in the guard class — tests written to prevent a specific defect from returning —
plus any test a removal decision depends on. A guard that survives its own mutation unchanged is
either rewritten to assert the thing it claims to, or removed with the surviving coverage named.
...
Each fact gets one canonical home, and the others carry a one-line summary and a pointer:

| Document | Owns |
|---|---|
| `README.md` | What tp is, install, the command surface, the flag inventory |
| `skills/tp/SKILL.md` | How an agent runs a cycle: workflows, decomposition, close recipes |
| `CLAUDE.md` | This repository's own conventions and its self-development loop |
| Reference | Exhaustive field, exit-code and schema detail |
...
They are added to their canonical home, and the flag inventory gains the same equality test as the
lint rules.
...
The recipe is documented in its canonical home. No feature changes; the instrument exists and the
axis was simply never named.

## 11. Non-Goals

1. **No behaviour changes outside the defects named here.** A sweep that also improves things is a
   sweep nobody can review. Every removal preserves observable behaviour, and the three write
   defects are the only intentional behaviour changes in this release.
...
11. Every guard test surviving §7 has a recorded mutation that makes it fail.
