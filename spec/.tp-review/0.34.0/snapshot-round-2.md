...
Every task in this release is one of two kinds, declared in its own acceptance: a **removal** task,
whose acceptance names what it removes and what still covers it, or a **repair** task, one of the
defects in §1 and §9. A task that is neither does not belong in this release.
...
The project gate becomes `go test -race ./... && golangci-lint run`, and `skills/tp/SKILL.md` — the
single home for it under §8.1 — recommends `-race` for any Go project using tp. The cost is real: `-race` roughly doubles test wall
...
Changing the project layer alone does not change what this release's own closes run. `tp init`
writes a `quality_gate` into each `<base>.tasks.json`, and a task-level value wins over the project
default, so every one of this repo's task files currently masks the project gate with a copy that
has no `-race`. The requirement is therefore stated on the resolved value: **the gate resolved for
every `spec/*.tasks.json` in this repository contains `-race`**, reached by removing the redundant
overrides rather than by editing each one — a subtraction that §2 already asks for.

A config-assertion guard test pins that resolved value, in the same spirit as the guard added in
v0.31.1 for the `gofmt` formatter. Asserting the project default alone would pass while every close
ran without `-race`.
...
`dupl` is enabled in `.golangci.yml` at its own default threshold, and the clones it reports are
either merged or exempted with a reason at the site. If merging a clone would require designing a
new abstraction to hold the merged form, Non-Goal 7 applies and the threshold is raised instead — to
the lowest value at which the tree is clean, recorded in `.golangci.yml` with the reason. The
recorded value is the specification; "small enough to act on" is not.

`.golangci.yml` is part of the shared quality gate, which runs at every close. Enabling `dupl` and
bringing the tree clean under it are therefore **one task**: split apart, the enabling task's own
`tp done` closes red on the first clone and every later task in the release inherits a red gate.
The clone count is discovered rather than planned, so that task's acceptance is "`golangci-lint run`
is clean with `dupl` enabled", not a number of merges.
...
Scope is a fixed list, not a description. Authorial intent is not an observable property of a test,
and this repository holds roughly 1100 test functions; "tests written to prevent a defect from
returning" is either all of them or none. The release's **first task** produces the list as its only
output: the guard tests added during the v0.31.2, v0.32.0 and v0.33.0 audit rounds, named
`file:function`, derived from those releases' commits. The sweep is capped at that list plus any
test a removal decision depends on.

Each guard that survives carries a one-line `// guard: <defect>` comment naming what it prevents,
which makes the class mechanically enumerable from then on. A guard that survives its own mutation
unchanged is either rewritten to assert the thing it claims to, or removed with the surviving
coverage named.
...
say "about 20". The engine gains an exported enumeration of its lint rule identifiers, the table is
checked against it, and the exact count is settled by that enumeration rather than by this sentence.
This is the durable half — a prose table that nothing checks will drift again within two releases.
...
### 9.3 `lint` and `init` name the spec, not the task file
...
1. **No behaviour changes outside the ones enumerated here.** This release intentionally changes
   observable behaviour in exactly five places: the three write defects of §1, the exit code in
   §9.1, and the hint text in §9.2 and §9.3, plus the gate command in §5. Everything in the three
   sweeps — §6, §7 and §8 — is behaviour-neutral, and a sweep that also improves things is a sweep
   nobody can review.
...
1. A second writer blocked on a held lock observes the first writer's committed result rather than
   overwriting it: the test holds the write lock, starts `tp import`, releases, and asserts the
   final file contains both writers' tasks. Deterministic — no repeat count, no tolerated failure
   rate.
...
11. The set of tests carrying a `// guard:` marker equals the surviving subset of §7.1's list, and
    no unlisted test carries the marker.
