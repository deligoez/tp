...
The observer for that rule is the first task's own acceptance. `scripts/check-measured-claims.py`
compares the written list against the derivation as a set, but it cannot distinguish a list that is
legitimately unwritten from one whose derivation failed — it reports `PENDING` until the list
exists. Closing that task with an empty list is therefore a decision a human makes, not one the
check can prevent.
...
Non-Goal 7 excludes; and observing emissions over this repository's own specs sees fewer than half
