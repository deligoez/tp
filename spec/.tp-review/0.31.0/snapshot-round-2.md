...
2. **Convergence-blocking unresolved findings present → resolve or verify them** (`tp review <file> --resolve-all <status> "<evidence>"` for the batch case, `--resolve <idx>` for one, or `tp review --verify <spec> --findings <file>` for disputed ones).
...
8.4 **`--compact` strips the advisory, keeps the decision-critical.** `next_action` and `harness_note` are omitted under `--compact` — the first is advisory and re-derivable, the second is free text that can run long. `harness_stale`, `nonblocking_open`, and `accepted_open` are single small values that change a decision, so they survive compaction, keeping the always-on `--compact` path free of the release's new prose while retaining the flags an agent acts on.
...
7. When a round is clean only because all surviving findings are below the blocking severities, `--status` and `--record` carry `nonblocking_open` (the count of surviving non-blocking findings) and `accepted_open: true` (§4.2).
...
14. `next_action` names exactly one command by the §8.2 precedence: a converged round names the forward command even when non-blocking findings remain open and unresolved; a round with convergence-blocking unresolved findings names resolve/verify; a round with a `mechanize_candidates` class present and none blocking names register-a-check. It gates no exit code (§8.1, §8.2).
...
17. An invalid `review_converge_on` value (via `tp set --workflow`, env, or project config) is rejected as a usage error (exit 2) with a hint naming the legal values, and a valid resolved layer is not shadowed by an invalid one (§3.3).
18. `tp review --status --compact` (and `--record --compact`) omit `next_action` and `harness_note` while retaining `harness_stale`, `nonblocking_open`, and `accepted_open` (§8.4).
