...
8.2 **What it names, in priority order.** `next_action` names exactly one command, chosen by a fixed precedence so a state satisfying several conditions is unambiguous:

1. **Converged → the phase's forward command** (review: decompose / import; audit: proceed). Convergence wins even when non-blocking findings remain open and unresolved: §4.2 makes leaving them open a legitimate, non-required choice, so `next_action` must not push "resolve them" at a converged round. For the purpose of the next trigger, "unresolved findings" means findings that *block* convergence under the effective `review_converge_on`; a non-blocking open finding never triggers it.
2. **Convergence-blocking unresolved findings present → resolve them.** `next_action` names one deterministic command: `tp review <file> --resolve-all <status> "<evidence>"` when more than one blocking finding is unresolved, `tp review <file> --resolve <idx> <status> "<evidence>"` when exactly one. `--verify` is a separate manual step an operator may run on a disputed finding; `next_action` never emits it.
3. **A `mechanize_candidates` class present → register a check** (`tp set --workflow checks='[…]'`). `mechanize_candidates` is the existing tp signal, not new here: `--record` already surfaces a `mechanize_candidates` array — a finding `class` that recurs across rounds — and `next_action` reads that same array.

For the audit loop the same precedence holds with audit's own commands, and only two states apply: a converged audit round names the forward command (proceed); a non-converged audit round (open non-PASS rows) names the fix-and-re-audit command — address the findings, then `tp audit <spec> --record <file>` for the next round. The resolve/verify and `mechanize_candidates` branches are review-only: audit findings are PASS/FAIL rows with no `--resolve`/`--verify` path, and audit `--record` surfaces no `mechanize_candidates`.

`next_action` is advisory and read-only: it changes nothing and never gates an exit code.
...
8.4 **`--compact` strips only the free-text note.** `harness_note` is omitted under `--compact` — it is free text that can run long. `next_action`, `harness_stale`, and `nonblocking_open` each survive compaction: they are single short, decision-critical values, and `next_action` in particular is the command that changes the agent's very next call — stripping it from the always-on `--compact` path would remove the field exactly where the default-compact agent needs it. `accepted_open` is not emitted at all; it is derivable from the round's clean flag and `nonblocking_open`.
...
5. A legacy round — one whose recorded findings carry no per-finding severity — is read without error and its stored clean flag is honored (§3.4).
6. A finding resolved `wontfix` with evidence does not block a round under either setting, including a critical one (§3.5).
7. When a round is clean only because all surviving findings are below the blocking severities, `--status` and `--record` carry `nonblocking_open > 0` alongside the round's clean flag, making the accepted-open state visible; `accepted_open` is not a separate emitted field (§4.2).
...
11. `--status` reports `harness_note` (the latest note) and `harness_stale`: true only when the two most recent rounds both carry non-empty differing notes; false when either note is empty (a predate or an opt-out) and when fewer than two rounds are recorded (§6.3).
...
14. `next_action` names exactly one command by the §8.2 precedence: a converged round names the forward command even when non-blocking findings remain open and unresolved; a round with more than one convergence-blocking unresolved finding names `--resolve-all`, one with exactly one names `--resolve <idx>` (never `--verify`); a round with a `mechanize_candidates` class present and none blocking names register-a-check. The audit `next_action` names the fix-and-re-audit command for a non-converged audit round and the forward command when converged (the resolve/verify and mechanize branches do not apply to audit). It gates no exit code (§8.1, §8.2).
...
17. An invalid `review_converge_on` value in the winning layer is rejected as a usage error (exit 2) with a hint naming the legal values; an invalid value in a lower, non-winning layer does not error when a higher-precedence layer holds a valid value (§3.3).
18. `tp review --status --compact` (and `--record --compact`) omit `harness_note` while retaining `next_action`, `harness_stale`, and `nonblocking_open`; `accepted_open` is not emitted (§8.4).
19. `go test ./...` and `golangci-lint run` are clean, and SKILL.md, CLAUDE.md, README.md, and REFERENCE.md carry the §9 updates.
