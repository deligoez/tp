...
**The ratios are the claim; no absolute byte count is quoted, and that is deliberate.** Round 5
attached three per-file-count figures here and round 6 could not reproduce two of them at any file
set — the audit ratio has at least three free inputs (which files, whether `--findings` is passed
and how large, the corpus), so every qualifier added is one more falsifiable claim. Measuring it
properly is a script, and `spec/0.40.0.md` is where measurement work belongs. An earlier draft gave the review figure with no round attached, repeating at small
scale the error §4.1 exists to name.
