...
green. Whether the isolation is `--no-state`, a temporary state directory or a copied tree is the
implementer's choice; what is fixed is the outcome: **`spec/.tp-review/` and `.tp/rounds/` are
byte-identical before and after the suite runs**, and that equality is itself asserted.
...
> without the clauses plus §2.3's suffix, and differs nowhere else.
...
**Breadth is the only thing this adds, and it is the part a fixture cannot supply.** A fixture has
the roles the fixture author thought of. The repository has specs with deactivated roles, with
domain frontmatter, with empty checklists, and at rounds where `regression` does and does not
emit — the five skip reasons of §4.2's table, occurring for real rather than by construction. A
suppression that only appears on one of those shapes passes every fixture test and fails here.
