...
A round in the walked segment that contributes no rows emits one advisory through `output.Notice`: `round <N> file <name> is missing; skipping its rows`, the wording the review record path already uses for the same condition, or `round <N> has no recorded rows file; skipping its rows` when the entry's `file` is empty. Such a round terminates the walk, so at most one such advisory can fire per invocation. It applies on both outputs named in §2.5.
...
`tp audit --status` already emits `overlap_report`, which also has one entry per role. It measures something else and neither replaces the other: `overlap_report` clusters non-PASS rows by `(item_id, category)` and reports how many clusters each role found alone versus shared, which is the trim-a-redundant-role signal. `role_streaks` counts rows, not clusters, and is the only place a per-round history appears.
...
| `open_roles` | the ids of the roles holding those rows, each id once, ascending; a row carrying no role contributes no id |
...
An entry in the effective workflow's `checks` is **valid** when `engine.ValidateChecks` accepts it **on its own** — the single-entry call `runMechanicalChecks` already makes for each entry it runs. Validity is judged per entry, never over the slice, so one invalid entry never changes the treatment of another entry's class, in either direction. The validator's duplicate-class rule is cross-entry and is therefore structurally unreachable in this form; a class named by two entries is simply registered, and `mechanized_classes` and the reviewer exclusion list of §3.2 each name it once. Neither state arises through tp: `tp set --workflow checks=`, `tp set --workflow --project` and `tp validate` all run the validator over the whole slice and reject both an invalid entry and a duplicate class, so a workflow containing either was hand-edited.
...
An entry the validator rejects is never executed, so it must not mechanize its class: it neither retires the suggestion that asked for a check to be written, nor tells a reviewer to stop reporting the class. Because tp's own write paths reject such an entry, a driver reaching this state edited the file by hand, and the class staying in `mechanize_candidates` is the correct report of it. tp adds no advisory for the condition.
...
**The reviewer exclusion list.** Prompt emission appends `Mechanically checked classes — do NOT report findings of these classes:` followed by the mechanized classes. Membership changes only in that an entry the validator rejects is dropped; a registered class that never reached candidate frequency stays on the list exactly as today, because this list is about what a reviewer should stop reporting, not about what tp is still suggesting. When the drop empties the list, the sentence is not appended at all.
...
`README.md` and `skills/tp/SKILL.md` must each contain both substrings:

- `audit convergence still counts every non-PASS row`
- `a registered check retires its mechanize candidate`

`CLAUDE.md`'s existing audit-scope rule must name `spec_coverage_clean_rounds` as the field carrying the number that rule currently tells the driver to track by hand.

`skills/tp/REFERENCE.md` is the field-level reference and carries the detail instead of the two summary substrings, which SKILL.md — loaded into every agent context alongside CLAUDE.md — already states. It must contain these five sentences verbatim, so the guard test asserts the document rather than the implementer's choice of anchor:
...
Sentence 5 is the `divergence.hint` constant of §2.6, so the one string a `--compact` reader would otherwise have no verbatim source for is pinned in a committed document.
...
11. `open_roles` names exactly the non-`spec-coverage` roles holding open rows, ascending, each id once over a fixture where one role holds three open rows, and omits an id for a row carrying no role.
...
17. `--compact` on both `--status` and `--record` omits `role_streaks` and keeps `spec_coverage_clean_rounds` and every field of `divergence` — including `message` and `hint` byte-identical to their non-compact values. Asserted over a fixture holding one non-PASS row with no role, so `unattributed_open` is present under both verbosities.
...
31. A guard test asserts `README.md` and `skills/tp/SKILL.md` each contain both required substrings of §4, that `CLAUDE.md` contains `spec_coverage_clean_rounds`, and that `skills/tp/REFERENCE.md` contains the five verbatim sentences.
