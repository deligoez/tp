...
1. **The divergence signal.** `tp audit --status` and `--record` report each auditor role's consecutive clean-round count, name `spec-coverage`'s separately, and say so explicitly when `spec-coverage` has been clean for the required number of rounds against an unchanged spec while other roles still hold open findings.
...
Two constraints hold throughout. Every rule that could fail in two directions fails toward "keep auditing": a state tp cannot measure never produces the signal that tells a driver the decision is theirs. And every command this release touches stays a function of the spec it is given plus the state recorded beside it — no new output depends on anything a repository does not commit.

## 2. The audit divergence signal

### 2.1 Reading a recorded round's rows

A recorded audit round's rows are loaded with the same loader the review side already uses. Three definitions govern everything in §2, and none may be restated differently anywhere else:
...
- **A round contributes no rows** when its entry's `file` is empty, or when `engine.LoadRoundRows` reports the recorded NDJSON file as not found — absent, or unreadable at the filesystem level. A file that reads but holds malformed lines does **not** take this path: that loader drops the malformed lines and returns the rest, and the surviving valid rows decide the round. This is the only meaning of "unreadable" in this spec.
...
A round in the walked segment that contributes no rows emits one advisory through `output.Notice`: `round <N> file <name> is missing; skipping its rows`, the wording the review record path already uses for the same condition, or `round <N> has no recorded rows file; skipping its rows` when the entry's `file` is empty. §2.2 bounds the walk so at most one such advisory can fire per invocation. It applies on both outputs named in §2.5.

`output.Notice` is silenced by `--quiet`, so an agent run with that flag does not see the advisory. That is accepted rather than worked around: a round contributing no rows can only make the divergence signal less likely to fire, never more, and the payload reports the same condition structurally through `spec_coverage_clean_rounds` and an empty `role_streaks`.
...
`role_streaks` is `[]` whenever no role appears in the latest recorded audit round. Four states reach it: no recorded round exists at all; the latest round contributes no rows; the latest round recorded zero rows; the latest round's rows all carry no role. The array does not distinguish them, and it is not the field that discloses them — `spec_coverage_clean_rounds` reports `null` in all four (§2.3), which is the statement a driver needs: this round did not measure conformance. Convergence arithmetic is independent of the array and continues to read the stored per-round `clean` flag, so `converged: true` beside an empty `role_streaks` is a reachable and correct combination.

A non-`spec-coverage` role's streak is reported because `open` alone describes only the current round. A lens that has been quiet for five rounds and one that broke a five-round streak this round are the same in `open` and different in `consecutive_clean`, and that difference is what tells a driver whether a repair has just regressed something or a known backlog is simply still there. For a role that holds open rows in the latest round the streak is necessarily 0, and carries no information beyond `open > 0`; its value is on the roles that are clean.

### 2.3 `spec-coverage` is reported by name

`spec_coverage_clean_rounds` is a top-level field on both outputs: the `consecutive_clean` of the `spec-coverage` role, or `null` when the **latest** recorded audit round holds no row attributed to that id — including when no round is recorded at all.
...
Conditions 1 and 3 mirror the two terms of `engine.Converged` — the clean-round count and the staleness guard — with the count taken over `spec-coverage`'s rows instead of over the stored per-round `clean` flags. The two are not equivalent, and this spec does not claim they are: a round that recorded zero rows, or whose file was later deleted, counts as clean for `engine.ConsecutiveClean` while ending every role's streak, so a history can converge without ever firing `divergence`. That asymmetry is the safe direction — it withholds the signal rather than inventing one — and §2.2 already states the visible half of it.

Condition 3 constrains `--status` only. On `--record` the round is stored with the spec hash computed in the same invocation, so `stale` is false there by construction; the condition is stated once, evaluated the same way on both, and must not be wired to a different staleness source on either.
...
The message is one of these two, with `round`/`rounds` and `finding`/`findings` agreeing with their numbers. The round count is `spec_coverage_clean_rounds`, never the `audit_clean_rounds` threshold it was compared against; the finding count is `other_roles_open`; the parenthetical count is `unattributed_open`:
...
Every field this release adds has a stated `--compact` disposition, and the table is the whole rule. A field kept under `--compact` is one a driver decides on; a field dropped is one that restates something already in the payload or in the documentation:

| Field | `--compact` | Why |
|---|---|---|
| `spec_coverage_clean_rounds` | kept | the number the release exists to surface |
| `divergence.other_roles_open` | kept | the size of the open set |
| `divergence.open_roles` | kept | the only attribution the object carries |
| `divergence.unattributed_open` | kept | qualifies the two above |
| `divergence.message` | dropped | restates the three fields above |
| `divergence.hint` | dropped | a fixed constant, also pinned verbatim in the documentation |
| `role_streaks` | dropped | per-role detail behind the decision |
| `mechanized_classes` | kept | names what vanished from a sibling array (§3.3) |

`divergence` itself is emitted under `--compact` whenever its three conditions hold; only the two string fields are stripped from it.
...
An entry in the effective workflow's `checks` is **valid** when `engine.ValidateChecks` accepts it **on its own** — the single-entry call `runMechanicalChecks` already makes for each entry it runs. Validity is judged per entry, never over the slice, and three consequences follow from that choice:

1. One invalid entry never changes the treatment of another entry's class, in either direction.
2. The validator's duplicate-class rule is cross-entry and is structurally unreachable in the single-entry form, so a class registered twice is treated as registered rather than rejected.
3. An entry the validator rejects is never executed, so it must not retire the suggestion that asked for a check to be written.

A finding class is **mechanized** when a valid `checks` entry's `class` equals it exactly. A mechanized class is excluded from `mechanize_candidates`. The frequency threshold — a class in at least 2 distinct rounds, or at least 5 times in one round — is unchanged, and the observable rule is that suppressing one class never changes whether another class crosses it.
...
*Mechanized* — the conjunction of "is a candidate" and "a valid entry names it" — governs candidate suppression, in two modes:
...
*Valid entry* — §3.1's per-entry rule on its own, with no candidate term — governs the reviewer-facing exclusion list that prompt emission appends, the one reading `Mechanically checked classes — do NOT report findings of these classes:`. That list is derived from the registered classes, not from candidates, and this release changes only which entries qualify: an entry the validator rejects is dropped from it, because telling a reviewer to stop reporting a class whose check never runs is the same defect pointing the other way. A registered-but-infrequent class stays on the list exactly as today. When the drop empties the list, the sentence is not appended at all.
...
`mechanized_classes` accompanies `mechanize_candidates` on `tp review <spec> --record <file>`, carrying the candidate classes withheld because a valid check is registered, sorted ascending. It is emitted only when non-empty, and it survives `--compact` alongside `mechanize_candidates`, which that flag has never stripped.
...
`REFERENCE.md` must additionally document the JSON shape of `role_streaks`, `spec_coverage_clean_rounds`, `divergence` and `mechanized_classes`, and must contain these four sentences verbatim, so the guard test asserts the document rather than the implementer's choice of anchor:
...
3. `role_streaks is omitted under --compact; spec_coverage_clean_rounds and divergence survive it, without divergence.message and divergence.hint.`
4. `mechanized_classes names the candidate classes withheld because a valid check is registered.`
...
9. **Suppression in `tp review --report`.** It takes NDJSON positionals and no spec, so its only possible workflow source is the ambient task-file discovery chain — whose highest-priority stored layer, `.tp/local.json`, is git-ignored and machine-local. The same `--report` invocation over the same files would then suppress different classes on a developer machine and in CI, and a command that is today a function of its arguments would become a function of the working directory. §1's second constraint forbids that. The loop reads `--record` and `--status`, which are spec-anchored and reproducible; `--report` keeps listing every candidate class, registered or not.
...
6. A round whose recorded file is deleted contributes no rows, ends every streak, and emits the `round <N> file <name> is missing; skipping its rows` advisory once; a round entry with an empty `file` emits the `round <N> has no recorded rows file; skipping its rows` form instead. A round file left in place but containing one malformed line keeps its valid rows, emits no such advisory, and its roles stay clean — the discriminating half against an implementation that treats any parse failure as unreadable.
7. The walk is bounded: over a history of five rounds whose second-oldest round file is deleted, and where every latest-round role has a non-clean round above it, the deleted round is never read — asserted by the absence of its advisory. An implementation walking the whole history emits it and fails.
8. `role_streaks` is `[]` in all four states of §2.2 — no recorded round; the latest round's file deleted; a latest round recorded with zero rows; a latest round whose every row carries no role — and `spec_coverage_clean_rounds` is `null` in each. The last two are the states that emit no advisory, so the field is the only disclosure.
9. `spec_coverage_clean_rounds` is `null` when the latest round holds no `spec-coverage` row even though an earlier round does, and `0` when the latest round holds one that is not PASS. The first half is the discriminating one against an implementation that scans the whole history for the role.
10. `divergence` fires when `spec-coverage` has been clean for the effective `audit_clean_rounds` and another role holds an open finding, and its `message` matches `spec-coverage clean 4 rounds; 2 findings open from other roles` verbatim over a fixture whose streak (4) exceeds the threshold (2) — pinning that the round slot carries the streak and not the threshold. A second fixture with one required clean round and one open finding asserts the singular `spec-coverage clean 1 round; 1 finding open from other roles`.
11. `open_roles` names exactly the non-`spec-coverage` roles holding open rows, ascending, and omits an id for a row carrying no role.
12. `divergence` is absent when `spec-coverage`'s streak is below the threshold, and absent when it meets the threshold with no other role holding an open finding.
13. The threshold is the effective `audit_clean_rounds`, not a literal 2: with `audit_clean_rounds` set to 3 and `spec-coverage` clean for exactly 2 rounds alongside another role's open finding, `divergence` is absent; a third clean round makes it fire.
14. A stale spec suppresses `divergence` on `--status` even when conditions 1 and 2 hold — the spec file is edited after the last round is recorded, `stale` reads true, and `divergence` is absent.
15. A non-PASS row carrying no `role` is counted in `other_roles_open`, disclosed in `unattributed_open`, and named inline in `message` with the `(including 1 with no role, which may be spec-coverage's)` suffix. With every non-`spec-coverage` open row attributed, `unattributed_open` is absent and the suffix does not appear.
16. `--record` computes the signal over the just-recorded round: recording a round in which `spec-coverage` completes its streak emits `divergence` in that same invocation's output, without a following `--status`.
17. `--compact` on both `--status` and `--record` follows §2.5's table exactly: `role_streaks`, `divergence.message` and `divergence.hint` are absent; `spec_coverage_clean_rounds`, `divergence.other_roles_open`, `divergence.open_roles` and `divergence.unattributed_open` are present.
18. The gate is unchanged: a fixture whose latest round holds only non-`spec-coverage` findings still reports `clean: false` and `converged: false`, `tp audit --status --check` still exits 1, and `next_action` is still the fix-and-re-audit directive. This is the guard test for §2.6 and must fail any implementation that lets the divergence signal reach convergence arithmetic.
19. `tp audit --merge` emits none of `role_streaks`, `spec_coverage_clean_rounds`, `divergence`.
20. A registered check suppresses its class: a class over the candidate threshold with a matching `checks` entry is absent from `mechanize_candidates` on `--record`, while an unregistered class over the same threshold is still listed.
21. Validity is per entry: a `checks` array holding one entry the validator rejects and one valid entry over a candidate class suppresses the valid entry's class and leaves the rejected entry's class listed. An implementation validating the whole slice suppresses neither and fails this test.
22. A class registered twice is mechanized rather than rejected, pinning §3.1 consequence 2 against an implementation that validates the slice and trips the duplicate-class rule.
23. Suppressing every candidate removes the register-a-check `hint` from `--record` output.
24. `tp review --status` honours the suppression through `next_action` on the same fixture, with no `mechanize_candidates` array of its own in the output. This is the second sink for the `next_action` path, and an implementation that filters only `--record`'s emitted array still names the mechanized class here.
25. A registered check whose command fails still suppresses its class, asserted with a check whose command exits non-zero.
26. `mechanized_classes` lists the suppressed classes sorted ascending, is absent when nothing was suppressed, does not list a registered class that never reached candidate frequency, and survives `--compact` on `--record`.
27. Prompt emission drops an invalid entry from the `Mechanically checked classes — do NOT report findings of these classes:` list while keeping a valid one, and keeps a valid entry whose class never reached candidate frequency — pinning that this list uses validity alone, not the *mechanized* conjunction.
28. When every registered entry is invalid, prompt emission appends no such sentence at all, rather than one ending in an empty list.
29. Both prompt-emission sites are covered: the multi-role panel and `tp review <spec> --perspective regression` each drop the invalid entry. The second is the discriminating half — an implementation guarding only the panel path passes the first and fails this.
30. `tp review --report` is unchanged: a class registered in a resolvable task file still appears in its `mechanize_candidates`, pinning Non-Goal 9 against an implementation that extends suppression there.
31. A guard test asserts `README.md`, `skills/tp/SKILL.md` and `skills/tp/REFERENCE.md` each contain both required substrings of §4, that `CLAUDE.md` contains `spec_coverage_clean_rounds`, and that `REFERENCE.md` contains the four verbatim sentences.
...
3. `internal/cli/review_test.go` and `internal/cli/review_regression_test.go` — contain the prompt-emission assertions for the `Mechanically checked classes` list at the two sites of §3.2. Any case there registering an entry the validator rejects is invalidated; cases registering only valid entries are not.
