...
This release removes the id-based dispatch and neutralizes the language-specific default auditor prompts. It is a patch: no new commands, no new flags, and no change to the audit result schema an auditor returns.
...
- `## Affected Files (10 of 34, max 10)` when it is greater — the list length, the pre-cap total, and the cap, in that order.
...
The inventory below is exhaustive against a search for every committed reference to `sel.Security`, `selectSecurity`, `HeadReader`, `roleSecurity`, `roleMaintainability`, `## Project Context`, and the literal `## Affected Files (max 20)`. Six committed tests are affected:
...
4. `TestFilterFiles_DropsBinaryFixturesDeleted` (`internal/engine/auditfiles_test.go`) asserts `assert.Empty(t, sel.Security)`. That one line references a removed field and is deleted, not renamed; the rest of the test stands.
5. The remaining `sel.Maintainability` and `MaintainabilityFileCap` references in `internal/engine/auditfiles_test.go` are renamed mechanically to `CodeFiles` and `CodeFileCap`.
6. `TestAuditPrompts_BodyOrderAndEmbedding` (`internal/cli/audit_prompts_test.go`) breaks twice: it locates the affected-files block by the literal `## Affected Files (max 20)`, which §2.6 rewrites, and it asserts `NotContains(sec, "## Project Context")`, which is precisely the id gate §2.3 removes. The literal lookup becomes a prefix match on `## Affected Files (`, the negative assertion for `security` is deleted, and the negative assertion for `spec-coverage` stands.
7. `TestGenerateAuditPrompts_CLAUDEmdOnlyForMaintainability` (`internal/cli/audit_named_test.go`) pins the id gate §2.3 removes. It is rewritten — and renamed to drop `OnlyForMaintainability` — to assert that every non-`spec-coverage` role's prompt contains the conventions marker and that `spec-coverage`'s does not.
...
§2 changes neither mechanism. The equality claim of §2.1 is therefore pinned against a fixture that holds both constant: **two separate audit invocations, each against a corpus whose only non-`spec-coverage` role is the one under test, with the two role ids of equal length** (for example `security` and `sekurity`). Both roles are then the inliner, both slugs truncate identically, and the two prompts must be byte-identical after replacing the role id string and the output path. Any surviving id branch anywhere in prompt construction fails that comparison.
...
In a repository where the keyword list already matched, the file count per prompt falls from up to 20 to up to 10, and two deltas push the other way: §2.3 adds up to 50 lines of project conventions to every non-`spec-coverage` prompt, replacing the resolve round-trips a convention-blind auditor generated, and §2.6 adds a short count to one header line. Net prompt size for such a role is roughly unchanged; the honest summary is that this release trades a narrower file list for a wider role panel, and does not claim to be free anywhere.
...
2. Two audit invocations, each against a corpus whose only non-`spec-coverage` role is the one under test, with equal-length role ids, produce byte-identical prompts after replacing the role id and the output path — the §2.9 fixture, which fails if any id branch survives anywhere in prompt construction.
...
5. A file whose path does not match a keyword but whose first lines contain `validate` or `auth` is ranked as an ordinary file, pinning that the content half of the heuristic is gone.
...
8. Every shared-arm prompt contains the `## Disposition` paragraph exactly once, and no `expected_evidence` value contains the word `PASS` — pinning §2.2's once-per-prompt placement rather than per-item repetition.
...
10. A role whose pre-cap total equals its list length renders `## Affected Files (max 10)`; a role whose pre-cap total is larger renders `## Affected Files (10 of 34, max 10)` with its own list length, total, and cap; a `spec-coverage` prompt renders `max 20`.
...
14. A guard test asserts that `README.md`, `skills/tp/SKILL.md`, `skills/tp/REFERENCE.md`, and `CLAUDE.md` each contain the substring `spec-coverage is the only reserved auditor id` and none contains `file-sec-` or `file-maint-`.
