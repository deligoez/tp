...
4.4 **Default output is text**, ready to paste into a sub-agent prompt. `--json` returns the brief's structured machine-readable parts only: `{task, prior_work, close, scope}`. The default (non-json) output is the assembled text brief; `--json` is for a driver that assembles its own prompt from those parts, so the assembled text is not duplicated.
...
5.4 `--prior <n>` overrides the recency count (default 5, range 0–20); an out-of-range value (<0 or >20) exits 2 (usage error). Dependency-derived entries are never dropped: a unit that builds on a prior task's output must see what that task produced. The section holds at most 12 entries in total; dependency-derived entries are always included even when they alone exceed 12, recency entries fill the remainder, and when entries are dropped the section says how many were omitted — a silent cap reads as "this is everything".
...
7.3 **Drift is visible, never blocked.** `commit_files` makes what a unit actually touched a matter of record: it appears in the next unit's prior work, in `tp show`, and in the audit phase's file selection. The audit phase reads a task's stored `commit_files` when present, falling back to deriving the paths from `commit_shas` when the field is absent (e.g. tasks closed before this release); the 50-path cap (§6.4) applies to the stored field. tp does not compare a commit's paths against the task's anchors — the mapping from a spec section to a source file is not derivable, and a wrong rejection at close time would be far more expensive than a visible record.
