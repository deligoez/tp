...
10.9 **Rounds recorded before this release keep their positional ids.** Every audit round tp records under v0.30.0+ carries an `id_scheme` marker — value `"slug"` — stating which id scheme its rows use. tp does not rewrite recorded round files, so a project mid-loop when it upgrades holds audit rounds recorded before this release that carry no marker: those are legacy rounds, whose rows use the old positional ids (`file-<role>-<n>`). The `id_scheme` marker is recorded on audit rounds only — the only rounds whose rows carry `(role, item_id)` and the only consumer; review rounds dedup on `(location, class)` and neither carry nor consume the marker. **The merge path is cross-scheme-safe by construction, not by marker check.** `tp audit --merge` deduplicates its input rows on `(role, item_id)` by string equality. A v0.30.0 slug id is `file-<role>-<slug>`, where the slug derives from a file path and the checklist text (§10.3); because that derivation starts from letters, the slug **always contains at least one letter** — it is never a bare integer. A legacy positional id is `file-<role>-<digits>` — bare digits after the role prefix — so the two shapes can never be string-equal: a positional id like `file-go-safety-2` names a row by its ordinal in that round, while a slug id like `file-go-safety-config-validation` names it by its subject. The merge's `(role, item_id)` string-equality dedup is therefore inherently cross-scheme-safe: it needs no `id_scheme` check on the result files it merges. Because v0.30.0 item ids are stable across rounds for the same subject (§10.3), merging rows from multiple slug-scheme rounds correctly deduplicates the same subject, and a legacy positional id simply never collides with any slug id. **The `id_scheme` marker's sole job is the prior-round section (§10.2)**: it tells tp to render a legacy (marker-less) round's prior-round rows with the "positional / not comparable" disclaimer — an honest gap is better than a false match between two ids that mean different things — so tp never presents a positional id and a slug id as if they named the same subject. Detection there is by the stored marker, not by parsing the id. The structural claim is testable: assert that every slug id contains at least one letter and that no slug id ever string-equals a positional id.
...
11.2 Tasks gain `duration_source`: `claimed` when `started_at` came from an explicit claim (`tp claim`, `tp next`, `tp next --brief`), `implicit` when it came from an implicit claim — which is both `tp done`'s implicit claim and `tp commit`'s (tp commit implicitly claims an open task as well). It is a managed field: `tp set` rejects it; `tp reopen` clears it. `tp report` carries it per task, excludes `implicit`-duration tasks from `estimation_accuracy` under a separate `implicit_duration` count, and also reports the `excluded_from_accuracy` count (tasks whose computed duration rounds to zero); the two buckets are disjoint — an `implicit`-duration task is counted only in `implicit_duration` even when its near-zero duration would round to 0.0, so `implicit_duration` takes precedence over `excluded_from_accuracy`. Both counts are reported, so an unmeasurable task is never confused with an instantaneous one. A task closed via `tp done --covered-by` is exempt from the implicit-claim rule above: it records no implementing work (§6.1) and has no meaningful `started_at`-to-`closed_at` span, so despite the `tp done` verb it is not classified as `implicit` (it has no implementing commit and is not a normal `tp done` close — §11.1's rule does not apply). `tp report` excludes `--covered-by` closes from `estimation_accuracy` directly, for the absence of a measured duration, reporting them in the existing `excluded_from_accuracy` count, separate from the `implicit_duration` count.
