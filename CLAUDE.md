...
- **v0.25.0 role corpus** — review/audit roles are project-owned data (`.tp/reviewers/*.json`, `.tp/auditors/*.json`, one file per role: `{id,title,instructions,focus[],domains[]}`, `id` = filename stem, `regression` reserved). tp OWNS the finding output contract; a role only customizes the prompt. `tp init --eject-roles [--domain software|prose] [--force]` writes the defaults as editable, byte-identical files, and `tp lint`/`tp review`/`tp audit` abort (exit 3) on a malformed role file. **tp's own repo stays on the embedded default corpus** (the three software reviewers + three auditors fit; ejecting would add role files with no benefit) — yet every `tp review`/`tp audit` still dogfoods the corpus emission, `(location,class)` dedup + per-role overlap report, and per-phase `roles_hash` staleness, since those paths run over the embedded corpus too. The standalone `tp: lens` frontmatter is retired (use `tp.review_roles`/`tp.audit_roles` focus overrides); `.tp-active` is removed (`.tp/local.json` is the sole active pointer).
