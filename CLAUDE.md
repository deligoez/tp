...
- **Prove a fix by running it, not by reading it — write the test first and watch it fail.** A test
  ...
  counting it as evidence. `spec/0.38.0.md` turns this into tp's evidence contract; until it ships it
  is a rule here.
...
visible, **`spec/0.37.0.md`** turns this file's remaining process rules into checks, and
**`spec/0.38.0.md`** — the evidence contract — makes tp stop accepting an assertion where an
experiment was meant. v0.36.0 goes first because it measures the loop that v0.37.0's other rules are
validated in; v0.38.0 goes last because its §7 composes into v0.37.0 §2's gate sequence. None of the
three has been reviewed.

**v0.38.0 came from field feedback**, not from this repo's own loop — a tp user driving a Rust NLP
project. Three of their seven reports are answered there (audit evidence is a keyword search, no
`UNVERIFIED` verdict, closure evidence unchecked for kind); `spec/0.36.0.md` §6.1 answers the
reconcile request and its Non-Goals 7-8 record what was deliberately not taken. Two more reports did
