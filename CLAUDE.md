...
- Quality gate after every task: `go test -race ./... && golangci-lint run` — this is the project gate in `.tp/config.json`, and no task file carries a `quality_gate` override that would mask it. tp's `.golangci.yml` enables the `gofmt` formatter.
